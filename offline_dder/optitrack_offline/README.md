...
The current measured material intervals begin with 0.060 m from the attachment
to `c1` and sum to 0.943 m. Saved model artifacts that report a 0.078-m first
interval or 0.961-m total length are historical results from the superseded
geometry. They are retained for provenance and must not be reported as fits of
the current cable geometry.
