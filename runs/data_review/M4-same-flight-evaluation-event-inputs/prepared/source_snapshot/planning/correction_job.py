...
@torch.no_grad()
def run(source,refdir,job,output,export,*,previous_rehearsal=None,iterations=30,strike_guard="none",strike_tolerance_m=0.):
    ...
    try:
        ...
        if previous_rehearsal is not None:
            ...
            with np.load(previous/'plan.npz') as z:baseline=tensor(z['position_control_points_m'])
        ...
        status('Checking unchanged M0 command under the frozen candidate model')
        ...
        summary=dict(schema='pva_fullstate_30hz_v1',planner='Local fixed-reference command correction',
            ...
            evidence='Simulation-only command correction toward the original M0 physical motion; Physical improvement has not been established.',
        ...
        (export/'README.md').write_text(
            ...
            'Commands are corrected with the frozen candidate model to track the original M0 predicted tip\n'
