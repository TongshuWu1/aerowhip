...
## Additional reported M2 conditions

The operator later reported slight quadrotor damage during M2 and suspects a
battery effect. Affected takes, damage timing and battery measurements are unknown.
Causation is unverified. All original metrics and all five takes remain unchanged.
See `runs/data_review/M2-paper-20260913/CONDITIONS_NOTE.md` and the batch's
`reported_conditions.json`. Earlier clean-take confirmation covered contact,
aborts and manual intervention, not independent verification of hardware condition.
