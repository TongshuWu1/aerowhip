...
class SimulatorMainWindow(QMainWindow):
    def __init__(self, project_root, model_config, task_config, ppo_config):
        ...
        self.mppi_page = CEMPage(project_root,optimizer='mppi')
        for page, (title, _) in zip((self.model_page, self.recordings_page, self.training_page,
                                    self.diagnostics_page, self.fullstate_page, self.cem_page, self.adaptation_check_page, self.mppi_page), PAGE_DEFINITIONS):
    ...
    def _page_changed(self, index):
        ...
        self.mppi_page.set_page_active(index == 7)
        if index==7:
    ...
    def closeEvent(self, event):
        self.closing = True
        ready = [page.shutdown() for page in (self.training_page, self.fullstate_page, self.cem_page, self.adaptation_check_page, self.mppi_page)]
        if not all(ready):
            event.ignore()
