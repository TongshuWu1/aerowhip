"""Native Windows eight-page UI and MPPI saved-result replay smoke test."""
...
if __name__=='__main__':
    ...
    assert window.main_tabs.count()==8
    assert window.main_tabs.tabText(6)=='Adaptation Check'
    page=window.mppi_page;window.main_tabs.setCurrentIndex(7);app.processEvents()
    ...
    for i in [0,6,7]:window.main_tabs.setCurrentIndex(i);app.processEvents()
    atomic_json(output/'verification.json',dict(pages=8,adaptation_check_index=6,
    ...
    print('Native eight-page UI, MPPI setup, VTK replay, rewind and PVA passed.')
