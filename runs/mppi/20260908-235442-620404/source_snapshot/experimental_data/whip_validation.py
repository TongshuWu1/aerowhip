...
@torch.no_grad()
def validate(job, *, hanging_only=False):
    torch.set_num_threads(1)
    settings=read(job/'protocol.json');data=read_inputs(job)
    out=job/('validation_hanging' if hanging_only else 'validation');out.mkdir(exist_ok=False)
    ...
    for i,names in enumerate(settings['folds'],1):
        ...
        if not hanging_only and not a['cable_fit_valid'][start-20:start+1].all():raise ValueError('Invalid initial cable history')
        ...
        if hanging_only:
            arc=tensor(np.r_[0.,np.cumsum(cable.rest_lengths_m)])
            q=args[0][:,None].expand(-1,cable.node_count,-1).clone();q[:,:,2]-=arc
            state=DderState(q,args[1][:,None].expand_as(q).clone())
        else:
        ...
        if hanging_only:labels,roots=['deployment_hanging_initial'],[corrected]
        variants=[replace(take,take_id=n,root_positions_m=r) for n,r in zip(labels,roots)]
        model.motion_residual=MaskedResidual(FrozenMotionResidual(payload['motion_residual']['checkpoint'],payload['motion_residual']['sha256']),tensor([1] if hanging_only else [0,1,0,1,1]))
        ...
        result=dict(metrics=metrics,observed_target=target_metrics(measured[:,-1],valid),start_frame=start,
            ...
            initial_cable_state='Assumed straight down, translational drone velocity' if hanging_only else 'Causal past marker observations',
...
if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--job',type=Path,required=True);parser.add_argument('--hanging-only',action='store_true')
    args=parser.parse_args();validate(args.job,hanging_only=args.hanging_only)
