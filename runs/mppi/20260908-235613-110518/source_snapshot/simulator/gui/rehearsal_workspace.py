...
class RehearsalWorkspace(QWidget):
    ...
    def load_result(self,directory):
        ...
        if self.metadata.get('schema') not in ('research_fullstate_30hz_v1','cem_fullstate_30hz_v1','mppi_fullstate_30hz_v1'):raise ValueError('Select a completed native 30 Hz rehearsal folder.')
        if self.metadata.get('planner') and not self.inspection_only:raise ValueError('Open spline results in their planner page → Rehearsal & Export.')
        ...
        self.result_label=((m['planner']+' spline' if m['planner'] in ('CEM','MPPI') else m['planner']) if m.get('planner') else 'Policy '+m['checkpoint_sha256'][:8])
    ...
    def draw_plots(self):
        ...
        self.plot_note.setText('Virtual force includes gravity; it is simulator input only.' if force else 'The planner optimizes a position spline. P/V/A are its analytic derivatives. Dashed position is fitted drone prediction. Recovery is not empirically validated.')
    ...
    def export_package(self):
        cem=self.metadata.get('schema') in ('cem_fullstate_30hz_v1','mppi_fullstate_30hz_v1')
