...
def generate(job,output,*,checkpoint=None,origin=None,target=None,device='cuda',progress=None):
    ...
    times,packets,phases,recovery=complete_packets(whip,cfg['launch']['origin_m'])
    ...
    output.mkdir(parents=True);saved=snapshot_assets(model,output)
...
def export_package(directory,destination):
    ...
    with zipfile.ZipFile(destination,'w',zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr('README.txt',meta['execution']+'\n\n'+meta['command_semantics']+'\n\n'+
            ...
            'This package contains commands, frozen model assets and policy/plan provenance. Regeneration uses the source snapshot in its originating research job.\n')
