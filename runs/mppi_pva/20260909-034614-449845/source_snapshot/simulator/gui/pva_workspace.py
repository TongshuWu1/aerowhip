...
def model_paths(root):
    ...
    for p in sorted((root/'runs/adaptation').glob('*/candidate/model.json'),reverse=True):paths.append(p)
...
class PVAPlannerPage(QWidget):
    ...
    def __init__(self,root,method):
        ...
        self.active=False;self.current_run=None;self.resume_checkpoint=None;self.fields={};self.last_history=None
        outer=QVBoxLayout(self);outer.setContentsMargins(18,12,18,14)
        self.tabs=QTabWidget();outer.addWidget(self.tabs)
        setup=QWidget();self.tabs.addTab(setup,'Setup & run');body=QVBoxLayout(setup)
        ...
        group=QGroupBox('Maneuver & strike');form=QFormLayout(group)
        self.horizon=QSpinBox();self.horizon.setRange(3,150);self.horizon.setValue(round(self.cfg['task']['duration_s']*30));form.addRow('30 Hz command intervals',self.horizon)
        ...
        for title,section in [('Rewards','reward'),('PVA limits','limits'),('PPO training' if method=='ppo' else 'MPPI sampling','training' if method=='ppo' else 'mppi')]:
            ...
            labels={
                ...
                'evaluate_every_updates':'Evaluate every N updates','samples':'Parallel candidate trajectories','iterations':'Iteration ceiling',
            ...
            for key,value in self.cfg[section].items():
                ...
                lower=0 if key in ('seed',) or isinstance(value,float) else 1
        ...
        rehearsal=QWidget();re=QVBoxLayout(rehearsal);self.tabs.addTab(rehearsal,'Rehearsal & export')
    ...
    def add_vector(self,layout,title,section,key,low,high,step,unit):
        ...
        for i,axis in enumerate('XYZ'):
            row.addWidget(QLabel(axis));spin=QDoubleSpinBox();spin.setDecimals(4);spin.setRange(low,high);spin.setSingleStep(step);spin.setValue(self.cfg[section][key][i]);spin.setFixedWidth(100);row.addWidget(spin);fields.append(spin)
    ...
    def collect(self):
        ...
        cfg['task'].update(duration_s=self.horizon.value()/30,first_contact_only=self.first_contact.isChecked())
    ...
    def describe_model(self):
        ...
        self.run.setEnabled(bool(m) and not (hasattr(self,'job') and self.job.running))
        ...
        self.model_note.setText((f'{provenance.get("label",self.models.currentText())}\nDrone {mass.get("drone_mass_kg",0)*1000:g} g · cable {mass.get("cable_assembly_mass_kg",0)*1000:g} g\n'+
            ('Normalized adp0 bootstrap; next flights provide prospective evidence.' if provenance.get('fit_complete') else 'Inspect the source and fit diagnostics in Models & fitting.')) if m else 'The selected bootstrap fit is not complete yet. Refresh models when its candidate is ready.')
    ...
    def refresh_library(self):
        ...
        for i,p in enumerate(self.runs):
            ...
            values=[identity.get('name',p.name),Path(identity.get('model_source','')).parent.name,status.get('status','unknown'),str(status.get('attempts',status.get('iteration',status.get('iterations','—')))),p.name]
            for j,v in enumerate(values):self.library.setItem(i,j,QTableWidgetItem(v))
        if self.runs:self.library.selectRow(0)
    ...
    def job_finished(self,code):
        self.run.setEnabled(True);self.refresh_library();self.poll()
    ...
    def poll(self):
        ...
        titles=['Episode return (batch mean)','Modeled valid hits','Closest tip distance','Infeasible attempts'] if self.method=='ppo' else ['Best sampled return','Modeled valid hits','Best sampled tip distance','Effective importance samples']
        for ax,key,title in zip(axes,keys,titles):
            factor=100 if key in ('success','failures') else 1;ax.plot(x,[r.get(key,0)*factor for r in rows],color='#2563eb',lw=1.6)
