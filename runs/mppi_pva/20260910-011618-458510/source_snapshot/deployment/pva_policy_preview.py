...
def generate_preview(job,output,*,device='cuda'):
    ...
    outcome='valid hit' if hit else 'infeasible attempt' if failed else 'invalid first contact' if bool(env.contact[0]) else 'miss / time limit'
    ...
    metadata=dict(schema='pva_policy_preview_v1',preview_only=True,planner='PPO policy preview',job=str(job),
        ...
        pullback=dict(required=cfg['task'].get('require_pullback',False)),
        wave=dict(required=cfg['task'].get('require_wave',False),completed_stages=int(env.wave_stage[0])),
