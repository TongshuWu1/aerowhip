...
class PVAModelPage(QWidget):
    ...
    def __init__(self,root):
        ...
        body.addWidget(note('Fresh unseen system: collect preliminary takes in Recordings, then review commands, timing and data roles before configuring the M0 fit. No old data or learned weights are selected.' if fresh else 'Current bootstrap: normalized flight batch only, with fresh nominal estimates and residuals.'))
    ...
    def inspect(self):
        ...
        rows=[('Drone response','Enabled' if m.get('fullstate_execution',{}).get('enabled') else 'Disabled','Effective cmdFullState tracking at OptiTrack origin'),
            ...
            ('Evidence', 'Retrospective fit diagnostics' if prov.get('fit_complete') else 'Historical model','Prospective flight performance is shown in Flight comparison')]
    ...
    def fit(self):
        ...
        self.active_fit=job;self.worker.start(self.root/'runs/model_jobs'/stamp(),command);self.fit_start.setEnabled(False)
    ...
    def poll(self,force=False):
        ...
        self.fit_start.setEnabled((status.get('status')=='prepared' if fresh else status.get('status')!='running') and not self.worker.running)
