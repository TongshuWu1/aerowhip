...
def optimize(job,model,cfg):
    ...
    controls=torch.einsum('ph,bhc->bpc',inverse,torch.atanh(bank[ids].clamp(-.999,.999)))
    controls[0]=inverse@torch.atanh(native.clamp(-.999,.999))
    seed_actions=torch.tanh(torch.einsum('hp,bpc->bhc',basis,controls))
    ...
    capture=ObjectiveCapture();env.branch_from(actual)
    ...
    while not s['iterations'] or iteration<s['iterations']:
        ...
        interpolated=torch.tanh(torch.einsum('hp,gbpc->gbhc',basis,sampled)).reshape(samples,steps,3)
        mean_actions=torch.tanh(torch.einsum('hp,gpc->ghc',basis,means))
        actions=torch.cat((interpolated,mean_actions,best_actions[None],native[None]))
        env.branch_from(actual);capture=ObjectiveCapture()
        result=env.rollout(actions=actions,observer=capture);score,terms=capture.score(env,result,actions,weights)
    ...
    check=ObjectiveCapture();verification=actual.rollout(actions=best_actions[None],observer=check,trace=True)
