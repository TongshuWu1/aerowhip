...
def calibrate_batch(root,batch):
    ...
    for name in flight_names(batch):
        ...
        for p in [mp,cp,mp.with_suffix('.tracking.json'),batch/'time_alignment.json',batch/'excluded_takes.json']:
...
def calibration_usable(result):
    review=result.get('normalization_review',{})
    return bool(result.get('checks_passed',False) or (
        result.get('mode')=='constant_per_take' and review.get('accepted') is True
        and review.get('acknowledged_remaining_drift') is True))


def load_calibration(batch,take=None):
    ...
    if not calibration_usable(result):raise ValueError('Batch height calibration needs review: '+'; '.join(result['warnings']))
    ...
    if result.get('mode')=='constant_per_take':
        biases=result.get('bias_by_take_m',{})
        if set(biases)!=expected or not all(np.isfinite(v) for v in biases.values()):
            raise ValueError('Missing or invalid per-take vertical bias.')
        if take is not None:
            if take not in biases:raise ValueError('Take has no vertical calibration.')
            result=dict(result,batch_reference_bias_z_m=result['bias_z_m'],bias_z_m=biases[take],selected_take=take)
    return result
...
def normalized_evaluation(data):
    ...
    if not calibration_usable(calibration) or not np.isfinite(calibration['bias_z_m']):
