...
def main():
    ...
    # Refusal must happen before the exception handler can write a status file.
    if args.stage=='checks' and out.exists():
        raise FileExistsError('Diagnostic already exists; choose a new output directory')
    if args.stage=='search' and (not (out/'forward_checks.json').exists() or (out/'evaluations.json').exists()):
        raise FileExistsError('Search requires reviewed checks and no previous search')
