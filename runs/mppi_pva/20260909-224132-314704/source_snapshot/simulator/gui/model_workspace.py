...
class RecordingsWorkspace(QWidget):
    def __init__(self,root):
        ...
        if read_json(Path(root)/'config/experiment.json',{}).get('schema')=='unseen_system_experiment_v1':
            from .preliminary_recordings_page import PreliminaryRecordingsPage
            self.preliminary=PreliminaryRecordingsPage(root);tabs.addTab(self.preliminary,'Preliminary takes')
