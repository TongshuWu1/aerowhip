...
class AdaptationProgressPage(QWidget):
    ...
    def __init__(self,root):
        ...
        self.study_controls=[row.itemAt(i).widget() for i in range(row.count()) if row.itemAt(i).widget() is not None]
        self.status=note('');layout.addWidget(self.status)
        self.summary=note('');layout.addWidget(self.summary)
        self.tabs=QTabWidget();layout.addWidget(self.tabs,1)
        from .real_flight_progress_page import RealFlightProgressPage
        self.real=RealFlightProgressPage(root);self.tabs.addTab(self.real,'Real flight progress')
        overview=QWidget();body=QVBoxLayout(overview);self.tabs.addTab(overview,'Model-fit diagnostics')
        ...
        self.fit_limits=note('');body.addWidget(self.fit_limits)
        trajectory=QWidget();body=QVBoxLayout(trajectory);self.tabs.addTab(trajectory,'Model-fit trajectories')
        ...
        self.tabs.currentChanged.connect(self.switch_tab);self.switch_tab(0)
        self.model_timer=QTimer(self);self.model_timer.setInterval(3000)
        self.model_timer.timeout.connect(lambda:self.update_model_status() if self.isVisible() else None);self.model_timer.start()

    def switch_tab(self,index):
        for widget in [*self.study_controls,self.status,self.summary]:widget.setVisible(index!=0)
    ...
    def load_selected(self,*_):
        ...
        try:
            ...
            self.summary.setText('MODEL-FIT DIAGNOSTIC ONLY: cross-validation on the existing adp0 recordings. '
                'These are not M1 real-flight results and do not populate the flight-progress chart.')
            if s['protocol'].get('vertical_calibration'):
                self.summary.setText('NORMALIZED MODEL-FIT DIAGNOSTIC: measured drone and cable Z are hover-normalized. '
                    'Each maneuver is excluded from its fit, but shared pre/post hover calibration uses all flights. '
                    'Conditional retrospective comparison; not independent validation or M1 real-flight improvement.')
            ...
            for axis,(key,label) in zip(axes,METRICS):
                ...
                axis.set(xticks=[0,1],xticklabels=['M0 fit check','Held-out fit'],title=label,ylabel='Prediction error [cm]');axis.grid(axis='y',alpha=.2)
            ...
            self.fit_limits.setText('Fit diagnostics: '+extra)
    ...
    def update_model_status(self):
        ...
        models=self.study['models'];policies=policies_for_model(self.root,models[0]) if len(models)==1 else []
        ...
        self.next_step.setText(stage+'\nReal M1 flight performance is unmeasured until adp1. Freeze the new policy/model/CSV before collection; evaluate the flights before fitting M2.')

    def shutdown(self):
        if not self.real.shutdown():return False
        self.model_timer.stop();return True
    ...
    def export_figure(self):
        ...
        if path:([self.real.figure,self.figure,self.trace_figure][self.tabs.currentIndex()]).savefig(path,dpi=300)
