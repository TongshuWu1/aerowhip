...
def select_drone_inputs(data,settings):
    names=settings.get('drone_response_takes',list(data))
    if not names or len(names)!=len(set(names)) or any(n not in data for n in names):
        raise ValueError('Drone response scope must name unique available recordings')
    selected={n:data[n] for n in names}
    folds=[[n for n in group if n in selected] for group in settings['folds']]
    folds=[group for group in folds if group]
    held=[n for group in folds for n in group]
    if sorted(held)!=sorted(names):raise ValueError('Drone development folds must hold out each selected take once')
    return selected,folds


def fit_drone(job, *, attachment=False):
    settings=read(job/'protocol.json');data,folds=select_drone_inputs(read_inputs(job),settings)
    rows=drone_windows(data,settings)
    ...
    atomic_json(output/'data_scope.json',dict(response_takes=list(data),folds=folds,
        scope='Only these recordings enter nominal response or residual fitting'))
    names=list(data);reviews=[]
    ...
    for fold,excluded in enumerate([*folds,[]]):
        ...
        if attachment:
            # An independently saved cable geometry can be frozen while the
            # command-response fit is restricted to another set of recordings.
            geometry_by_fold=settings.get('drone_attachment_geometry_by_fold')
            attachment_geometry=deepcopy(geometry_by_fold[label] if geometry_by_fold is not None
                                         else settings.get('drone_attachment_geometry'))
            if attachment_geometry is None and settings.get('drone_response_takes') and (job/'cable_result.json').exists():
                cable_directory=read(job/'cable_result.json')['directory']
                attachment_geometry=read(job/cable_directory/label/'geometry.json')
