...
class FullStatePage(TestingPage):
    ...
    def __init__(self, root):
        ...
        self.assumption.setText(self.assumption.text()+
            ' Hover XYZ is the cable attachment, below the top tracking origin. '
            'Any cf_7 export uses a fixed initial offset; drone tilt is not simulated here.')
    ...
    def show_csv_selection(self):
        ...
        if meta.get('reference_point')=='OptiTrack_cf7_origin':
            ...
            if meta.get('reference_mapping',{}).get('mode')=='fixed_initial_translation':
                self.plan_note.setText(self.plan_note.text()+
                    ' Mapping uses a fixed initial offset; it does not predict rotation of the attachment.')
