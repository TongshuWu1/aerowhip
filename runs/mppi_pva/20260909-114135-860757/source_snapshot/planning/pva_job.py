...
def validate_settings(cfg):
    ...
    for key in ('samples','iterations','minimum_iterations','patience'):
        value=cfg['mppi'][key]
        if isinstance(value,bool) or not isinstance(value,int) or value<1:raise ValueError('MPPI '+key+' must be a positive integer')
    if cfg['mppi']['minimum_iterations']>cfg['mppi']['iterations']:raise ValueError('MPPI minimum iterations exceeds its ceiling')
...
def mppi(job,model,cfg):
    ...
    for iteration in range(1,s['iterations']+1):
        ...
        row=dict(iteration=iteration,best_reward=best,batch_reward=float(result['reward'].mean()),
            ...
            failures=float(result['failed'].double().mean()),elapsed_s=time.perf_counter()-start,**best_metrics)
        history.append(row);atomic_json(job/'history.json',history)
        ...
        temporary.replace(job/'plan.npz')
