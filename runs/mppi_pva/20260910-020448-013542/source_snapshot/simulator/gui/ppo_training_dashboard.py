...
class PPOTrainingDashboard(QWidget):
    ...
    def update_run(self, status, settings, rows):
        ...
        if data['tip_contact'] is not None:
            outcome += f" Tip touched target: {percent(data['tip_contact'])}; valid hits require all whip criteria at first contact."
        elif data['closest_distance'] is not None:
        ...
        self.phase_title.setText('Policy test' + (f' at {tested_at:,} attempts' if tested_at is not None else '') + ' · independent criteria')
        ...
        self.detail.setText(stage + rate + 'Simulation development scenarios · latest saved updates')
