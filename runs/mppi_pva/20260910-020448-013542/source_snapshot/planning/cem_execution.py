...
from .spline import decode, sample, spline_feasible
from .cem_objective import reward_components
...
class SplineEvaluator:
    ...
    def __call__(self, vectors):
        ...
        for c, duration in splines:
            ...
            dense_valid.append(spline_feasible(c,duration,self.model['fullstate_execution']['feasibility'],
                (self.settings['minimum_height_m'],self.settings['maximum_height_m'])))
        ...
        components=reward_components(result,[d for _,d in splines],jerk,
            self.task['success']['minimum_directed_tip_speed_m_s'],self.settings.get('reward'))
        scores=sum(components.values())
        scores[~valid] = -np.inf
        return scores, dict(success_fraction=float(np.mean(result['success'] & valid)),
                            reference_feasible_fraction=float(np.mean(dense_valid)),
                            prediction_feasible_fraction=float(np.mean(~result['failed'])),
