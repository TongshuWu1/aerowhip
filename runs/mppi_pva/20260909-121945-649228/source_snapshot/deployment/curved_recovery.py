...
def plan_curved_recovery(position, velocity, acceleration, hover, settings=None):
    ...
    if moving:
        ...
        if not permitted(approach,approach_s,options,tilt_limit)[0]:
    ...
    for frames in range(int(np.ceil(options['minimum_turn_s']*30)),int(np.floor(options['maximum_turn_s']*30))+1):
        ...
        if ok:candidates.append((m['peak_height_m'],duration,c,m))
    if not candidates:
    ...
    return sample,dict(schema='curved_moving_recovery_v1',settings=options,
