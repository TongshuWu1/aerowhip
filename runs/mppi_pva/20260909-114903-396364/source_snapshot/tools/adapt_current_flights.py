...
if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('stage',choices=['prepare','drone','cable_batched','baseline','attitude_refine','validate']);p.add_argument('--job',type=Path,default=JOB)
    args=p.parse_args();torch.set_num_threads(1)
    {'prepare':prepare,'drone':drone_run,'cable_batched':cable_batched,'baseline':baseline_run,'attitude_refine':attitude_refine,'validate':validation_run}[args.stage](args.job)
