...
def main():
    ...
    parser.add_argument('--eager-backward',action='store_true')
    ...
    if args.eager_backward:
        ...
        constants=e.physics.runtime_constants(data['q']);dt=data['q'].new_full((2,),e.dt_s)
        def advance(q,v,roots):
            qs=[];vs=[]
            for root in roots.unbind(1):
                state=research_physics_step(e.physics,DderState(q,v),root,dt,constants,
                    create_graph=True,fast_solve=not args.dense,fast_geometry=True)
                q,v=state.positions_m,state.velocities_m_s;qs.append(q);vs.append(v)
            return torch.stack(qs,1),torch.stack(vs,1)
        q,v=data['q'],data['v'];qs=[q[:,None]];vs=[v[:,None]]
        for i in range(1,data['roots'].shape[1],3):
            qp,vp=checkpoint(advance,q,v,data['roots'][:,i:i+3],use_reentrant=False)
            q,v=qp[:,-1],vp[:,-1];qs.append(qp);vs.append(vp)
        q,v=torch.cat(qs,1),torch.cat(vs,1)
    else:
        q,v=accelerator(data,gradients=True)
    loss=cable_objectives(q,data['truth'],ids).mean();loss.backward()
    ...
    row=dict(autograd=ad,bias_index=index,loss=float(loss.detach()),gradient_norm=norm,
