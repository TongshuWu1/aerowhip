...
parent=ROOT/'runs/mppi_pva/20260915-135642-691893'
original=parent/'release_review'
saved=json.loads((parent/'result.json').read_text())
cfg=json.loads((parent/'settings.json').read_text())
shift=1.45-saved['selected_strike_position_m'][2]
cfg['launch']['origin_m'][2]+=shift
cfg['launch']['target_m'][2]+=shift
job,_=prepare(ROOT,cfg,'M5 forward-left horizontal whip - release at 1.45 m',
    development_review='User requested a lower maneuver with release around 1.45 m. Translate the existing command vertically, retain normalized jerk and timing, and independently replay all existing requirements and full recovery. No new MPPI search or target accuracy claim.')
...
try:
    ...
    with torch.no_grad():
        ...
        with np.load(original/'motion.npz') as z:
        ...
        assert abs(float(env.strike_position[0,2])-1.45)<1e-8
    ...
    atomic_json(job/'height_translation.json',dict(parent_job=str(parent),parent_rehearsal=str(original),
        ...
        height_shift_m=shift,desired_release_height_m=1.45,initial_hover_m=cfg['launch']['origin_m'],
        normalized_jerk_unchanged=True,timing_unchanged=True,position_max_difference_m=qerror,velocity_max_difference_m_s=verror))
    atomic_json(ROOT/'tmp/m5_horizontal_left_lowered.json',dict(job=str(job),output=str(ROOT/'runs/rehearsals_pva/M5-Forward-Left-Horizontal-Whip')))
