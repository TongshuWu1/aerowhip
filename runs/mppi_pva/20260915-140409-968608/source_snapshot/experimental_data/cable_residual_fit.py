...
PRELIMINARY_TAKES = {'fig8_001', 'fig8_002', 'fig8_003', 'fig8vertical_001', 'fig8vertical_002',
...
def residual_mode(settings):
    mode = settings.get('drag_mode', 'structured' if settings.get('learn_drag', False) else 'fixed')
    if mode not in ('nn_only', 'structured', 'fixed'):
        raise ValueError(f'Unknown residual drag mode: {mode}')
    return mode


def residual_base_model(payload, settings):
    result = deepcopy(payload)
    if residual_mode(settings) != 'fixed':
        result['cable']['external_drag_s_inv'] = 0.0
    return result


def make_residual(cable, payload, settings):
    structured = residual_mode(settings) == 'structured'
    return MotionResidual(cable.node_count, hidden=settings['hidden'],
        acceleration_limit=settings['acceleration_limit_m_s2'], learn_drag=structured,
        initial_drag_s_inv=payload['cable']['external_drag_s_inv'] if structured else 0.0)
...
def permitted_takes(manifest):
    ...
    for name, row in manifest['takes'].items():
        ...
        if ((name == 'fig8vertical_002' and not manifest.get('historical_fit_authorized',False))
...
def prepare_residual_job(root, *, updates=None):
    ...
    selected = permitted_takes(manifest)
    selected_manifest = dict(takes=selected)
    if manifest.get('historical_fit_authorized'):
        selected_manifest['historical_fit_authorized'] = True
    ...
    atomic_json(job/'dataset_manifest.json', selected_manifest)
    hashes = {}
    ...
    atomic_json(job/'provenance.json', dict(model_sha256=canonical_json_hash(model),
        manifest_sha256=canonical_json_hash(selected_manifest), settings_sha256=canonical_json_hash(settings),
        input_sha256=hashes, source_sha256=code_hashes, protected_test_used=False,
        formerly_protected_take_used='fig8vertical_002' in selected,
        whip_data_used=False, drone_residual_trained=False,
        physics_frozen=residual_mode(settings) == 'fixed',
        learned_drag=residual_mode(settings) == 'structured', drag_mode=residual_mode(settings),
...
def gradient_check(item, model, cable, parameters, network, *, epsilon=1e-3):
...
def fit(job):
    ...
    fit_payload = residual_base_model(payload, settings)
    cable, model, training, windows = prepare_takes(job, fit_payload, settings['training_horizon_s'],
        settings['training_windows_per_take'], device, training_only=True)
    network = make_residual(cable, payload, settings).to(device=device, dtype=torch.float64)
    ...
    for horizon in settings['evaluation_horizons_s']:
        ...
        _, _, takes, records = prepare_takes(job, fit_payload, horizon, settings['evaluation_windows_per_take'], device)
    ...
    candidate = deepcopy(fit_payload)
    candidate['motion_residual'] = dict(enabled=True, checkpoint=str(job/'residual_candidate.pt'),
        ...
        drag_mode=residual_mode(settings))
    ...
    atomic_json(job/'review.json', dict(accepted=accepted and best_update>0, validation_checks=checks,
        ...
        physics_frozen=residual_mode(settings) == 'fixed', learned_drag=network.learn_drag,
        drag_mode=residual_mode(settings), baseline_training_objective=baseline_loss,
        trainable_parameter_count=sum(p.numel() for p in network.parameters()),
        initial_drag_s_inv=payload['cable']['external_drag_s_inv'] if network.learn_drag else None,
