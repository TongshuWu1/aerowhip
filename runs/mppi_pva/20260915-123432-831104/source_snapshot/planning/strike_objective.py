...
def validate_settings(cfg):
    ...
    optional = {'lateral_weight', 'lateral_scale_m', 'speed_proximity_scale_m', 'maximum_strike_angle_deg', 'prefer_aligned_strike', 'speed_metric', 'minimum_tip_speed_gain_m_s', 'horizontal_cable_weight', 'horizontal_cable_scale_m', 'free_target', 'pullback', 'minimum_release_time_s', 'velocity_propagation', 'curved_release'}
...
def observe(env, previous, current, eligible, clock_left):
    ...
    for event_fraction in fractions:
        ...
        if free_target:
            ...
            env.whip_aligned_duration=torch.where(eligible & forward & speed_allowed,
                env.whip_aligned_duration+env.dt,torch.zeros_like(env.total))
