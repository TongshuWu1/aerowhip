"""Independent 30 Hz force sequences evaluated through the shared physical simulator."""
import numpy as np
import torch
...
class ForceEvaluator:
    ...
    def evaluate(self,vectors,record=False):
        ...
        # Shared force conversion, with MPPI-owned gravity and force limits.
        ...
        # Shared task equations with MPPI-owned weights, including failure costs.
    ...
    def hover_actions(self):
        """Zero normalized input maps to exact hanging hover, without a policy."""
        return np.zeros(self.steps*3,dtype=float)
