"""Calibration, PPO and offline MPPI trajectory research interface."""
...
from .model_workspace import RecordingsWorkspace,DiagnosticsPage
...
PAGE_DEFINITIONS = (
    ('Model', 'Inspect model versions, prepare adaptation jobs and review fit evidence'),
    ...
    ('Diagnostics', 'Historical development diagnostics; current flights are in Adaptation Check'),
    ('Rehearsal & Export', 'Inspect the frozen whip and complete 30 Hz FullState trajectory'),
    ('MPPI Planner', 'Independently optimize 30 Hz XYZ forces and export a complete FullState trajectory'),
    ('Adaptation Check', 'Replay measured flights beside their saved predicted drone and cable motion'),
)


class HistoricalResearchWindow(QMainWindow):
    def __init__(self, project_root, model_config, task_config, ppo_config):
        ...
        footnote = QLabel('30 Hz experiments\nModel saved with each run\nDrone + cable · both NNs')
        ...
        self.model_page = ModelHub(project_root)
        self.model_page.model_requested.connect(self.select_adapted_training_model)
        self.model_page.page_requested.connect(self.main_tabs.setCurrentIndex)
        ...
        from .mppi_page import MPPIPage
        self.mppi_page = MPPIPage(project_root)
        ...
        self.adaptation_check_page.progress_page.model_requested.connect(self.select_adapted_training_model)
        for page, (title, _) in zip((self.model_page, self.recordings_page, self.training_page,
                                    self.diagnostics_page, self.fullstate_page, self.mppi_page, self.adaptation_check_page), PAGE_DEFINITIONS):
    ...
    def select_adapted_training_model(self,path):
        page=self.training_page;index=page.training_model.findData(path)
        if index<0:page.training_model.addItem(Path(path).parent.name,path);index=page.training_model.count()-1
        page.training_model.setCurrentIndex(index);page.setup_button.setChecked(True)
        label='M0' if Path(path).resolve()==(self.project_root/'config/research_30hz/model.json').resolve() else Path(path).parent.name
        page.run_name.setText(label+' PPO - original flight setup')
        self.main_tabs.setCurrentIndex(2);page.tabs.setCurrentIndex(0)
    ...
    def open_flight_comparison(self,batch,take):
        ...
        page.batches.setCurrentIndex(i)
        page.takes.setCurrentIndex(page.takes.findText(take))
        page.views.setCurrentIndex(0);self.main_tabs.setCurrentIndex(6)
    ...
    def update_context(self):
        from simulator.workflow import read_json
        if self.main_tabs.currentIndex()==5:
            config=read_json(self.project_root/'config/mppi.json',{})
            self.model_status.setText('Independent MPPI setup · model: '+str(config.get('model_path','not selected'))+' · saved runs keep their frozen configuration')
        else:
            pointer=read_json(self.project_root/'config/research_workspace.json',{})
            self.model_status.setText('PPO workspace: '+pointer.get('model_label','not selected')+' · 30 Hz · saved policies and flight ghosts retain their own model')
    ...
    def _page_changed(self, index):
        ...
        self.mppi_page.set_page_active(index == 5)
    ...
    def closeEvent(self, event):
        self.closing = True
        ready = [page.shutdown() for page in (self.model_page,self.training_page, self.fullstate_page, self.mppi_page, self.adaptation_check_page)]
        if not all(ready):
            event.ignore()
...
# New sessions use the direct PVA workflow. Keep historical implementation
# available for source provenance; it is never the default launcher.
from .pva_main_window import PVAResearchWindow as SimulatorMainWindow
