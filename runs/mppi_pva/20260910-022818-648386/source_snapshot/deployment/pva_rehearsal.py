...
def generate(job,output,*,checkpoint=None,origin=None,target=None,device='cuda',progress=None):
    ...
    if cfg['task'].get('require_pullback',False):
        ...
        metadata['pullback']=dict(required=cfg['task'].get('success_criterion','legacy_strike_v1')!='tip_contact_v1',pull_completed=bool(env.pull_ready[0]),reverse_completed=bool(env.reverse_ready[0]),
    ...
    if cfg['task'].get('require_wave',False):
        metadata['wave']=dict(required=cfg['task'].get('success_criterion','legacy_strike_v1')!='tip_contact_v1',completed_stages=int(env.wave_stage[0]),
