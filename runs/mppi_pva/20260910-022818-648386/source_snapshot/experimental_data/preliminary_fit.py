...
def publish(job,trials,records):
    ...
    for take in sorted(results):
        ...
        for label,engine in engines.items():
            ...
            if label=='fitted':
                from .bootstrap_drone import TranslationBatch,trial_args
                from simulator.drone_pose_response import predict_pose
                with torch.no_grad():reference,_=TranslationBatch([t],engine.drone.parameters).predict(engine.drone.residual)
                checked=predict_pose(**trial_args(t,engine.drone.parameters),residual=engine.drone.residual)
                exact_error=float((reference[0]-checked['position_origin_m'][0]).abs().max())
                runtime=pose_prediction(t,engine.drone,t.time)
                error=float((reference[0]-runtime['position_origin_m'][0]).abs().max())
                if exact_error>1e-10 or error>1e-4 or not bool(runtime['valid'].all()):raise AssertionError('Training/runtime drone mismatch')
                consistency[take]=dict(training_vs_reference_difference_m=exact_error,production_difference_m=error,
                    production_tolerance_m=1e-4,reason='Production removes floating-point ceil over-subdivision at exact 5 ms boundaries; reference retains the exact fit partition')
