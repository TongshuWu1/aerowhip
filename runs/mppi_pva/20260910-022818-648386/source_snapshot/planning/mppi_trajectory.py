...
def optimize(job,model,cfg):
    ...
    if timed:
        ...
        means[:2].zero_()
        atomic_json(job/'baseline_evaluation.json',dict(names=['liked_old_commands_new_M0','previous_new_M0_sweep'],
    ...
    while not s['iterations'] or iteration<s['iterations']:
        ...
        if timed and best+1e-7<max(baseline_scores):raise ValueError('Known baseline score was lost')
        ...
        if cfg.get('visualization',{}).get('live_mppi',True):
            ...
            if timed:labels.update({samples+groups:'Incumbent',len(score)-2:'Old commands / new M0 baseline',len(score)-1:'Previous sweep baseline'})
