...
def series(capture,index,iteration,score,result,label):
    ...
    return dict(arrays,iteration=iteration,candidate_id=index,score=float(score[index]),
...
def publish(job,actual,capture,score,result,best,iteration):
    ...
    rows=[best]+[series(capture,i,iteration,score,result,'Evaluated mean' if i==count-1 else f'Sample {i}') for i in ids
