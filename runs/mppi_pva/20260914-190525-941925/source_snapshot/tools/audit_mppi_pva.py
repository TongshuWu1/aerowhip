...
def main():
    ...
    parser.add_argument('--settings',type=Path,help='Explicit diagnostic settings; active defaults remain unchanged')
    parser.add_argument('--continue-prefix-job',type=Path,help='Preserve a verified prefix, then optimize the remaining release with current code')
    parser.add_argument('--prefix-steps',type=int,default=30)
    args=parser.parse_args();out=args.output.resolve();out.mkdir(parents=True,exist_ok=False)
    cfg=read_json(args.settings) if args.settings else load_settings(ROOT,'mppi')
    ...
    if args.continue_prefix_job:
        parent=args.continue_prefix_job.resolve();previous=read_json(parent/'settings.json')
        if read_json(parent/'status.json')['status']!='completed':raise ValueError('Prefix parent must be complete')
        for key in ('launch','action','limits','reward','task','mppi'):
            if previous[key]!=cfg[key]:raise ValueError('Prefix continuation settings differ: '+key)
        if read_json(parent/'identity.json')['model_source_sha256']!=report['model_sha256']:raise ValueError('Prefix model differs')
        with np.load(parent/'plan.npz') as data:original=data['normalized_jerk'].copy()
        count=args.prefix_steps;horizon=round(cfg['mppi']['horizon_s']*30)
        if not 0<count<len(original):raise ValueError('Preserve a strict prefix of the completed parent')
        proposal=np.zeros((horizon,3));tail=original[count:count+horizon];proposal[:len(tail)]=tail
        np.savez_compressed(job/'continuation.npz',prefix=original[:count],proposal=proposal)
        report.update(prefix_parent=str(parent),preserved_command_steps=count,parent_plan_sha256=sha256_file(parent/'plan.npz'))
        identity=read_json(job/'identity.json');identity.update(name='MPPI pullback · preserved forward pull · refined release',
            prefix_parent=str(parent),preserved_command_steps=count,parent_plan_sha256=report['parent_plan_sha256'])
        atomic_json(job/'identity.json',identity);atomic_json(out/'result.json',report)
    if args.replay_job:
        ...
        if (parent/'windows.json').exists():shutil.copy2(parent/'windows.json',job/'windows.json')
