...
def classify_execution(ct, values, fresh, age, profile):
    ...
    coverage = last_callback-float(ct[end]) if last_callback is not None else None
    warnings = []
    if coverage is not None and abs(coverage-profile['source_constants']['RECOVERY_TIME']) > .2:
        warnings.append('Observed post-hold callback coverage differs from supplied RECOVERY_TIME; do not infer the missing interval or installed script revision.')
    report = dict(schema='legacy_whip_phases_v1', csv_start_s=float(ct[first]), csv_end_s=float(ct[end]),
        ...
        post_callback_coverage_s=coverage,
        ...
        cache_tail='A fresh cached value can outlive publishing by up to 100 ms. Phase end is an observation limit, not a verified mode-switch time.',
        warnings=warnings)
