...
def recorded_packets(c,*,invalid_rows_break_coverage=False):
    ...
    if not len(ids):raise ValueError('No valid recorded command receipts')
    ...
    if invalid_rows_break_coverage:
        ...
        if len(invalid_time):
            index=np.searchsorted(invalid_time,times,side='left')
            cutoff=np.r_[invalid_time,np.inf][index]
            valid_until=np.minimum(valid_until,cutoff)
            if np.any(valid_until<=times):raise ValueError('Conflicting valid/invalid command receipt; review logger timing')
