...
def render(output,arrays,metadata,settings,*,animate=True):
    ...
    figure=Figure(figsize=(11,9),layout='constrained');axes=figure.subplots(3,1)
    ...
    axes[0].set_aspect('equal',adjustable='datalim');axes[0].legend(ncol=4,fontsize=8)
    mesh=axes[1].pcolormesh(t,material,angles.T,shading='nearest',cmap='magma')
    axes[1].plot(t,location,color='cyan',lw=1,label='Dominant bend')
...
if __name__=='__main__':
    ...
    if args.job:
        arrays,metadata,settings=replay(args.job)
