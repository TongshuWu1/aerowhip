...
class PVAPlannerPage(QWidget):
    ...
    def __init__(self,root,method):
        ...
        self.spline_setup=method=='mppi' and read_json(self.root/'config/pva/systematic_strike.json',{}).get('command_contract')=='position_spline_pva_30hz_v1'
        if self.spline_setup:
            from .spline_setup import build
            self.cfg=read_json(self.root/'config/pva/systematic_strike.json');build(self)
        else:
            ...
            from learning.ppo_trajectory_reward import enabled
            reward_section='trajectory_objective' if enabled(self.cfg) else 'reward'
            for title,section in [('Rewards',reward_section),('PVA limits','limits'),('PPO training' if method=='ppo' else 'MPPI sampling','training' if method=='ppo' else 'mppi')]:
                ...
                labels={
                    ...
                    'early_hit':'Earlier successful hit · maximum bonus','early_hit_scale_s':'Earlier-hit decay time [s]',
                    'impact':'Harder tip hit · maximum bonus','impact_scale_m_s':'Impact speed scale [m/s]',
                ...
                for key,value in self.cfg[section].items():
                    if section=='trajectory_objective' and isinstance(value,str):continue
                ...
                if section=='trajectory_objective':v.addWidget(note('Shared MPPI whole-trajectory score: preferred fold, contact, cast and effort. Tip contact defines success. Failed trajectories receive a finite PPO penalty. Recovery is not scored.'))
        ...
        if method=='ppo':
            ...
            self.preview_complete=QCheckBox('Include recovery + CSV');self.preview_complete.setChecked(True)
            self.preview_complete.setToolTip('Append the same smooth braking, return to start and final hover used by MPPI. Uncheck for a whip-only preview of an unfinished policy.')
            preview_row.addWidget(self.preview_complete)
        ...
        if method=='mppi':self.progress_note.hide();self.canvas.setMinimumHeight(240);self.canvas.setMaximumHeight(320)
    ...
    def save_settings(self):
        try:self.cfg=self.collect();atomic_json(self.root/'config/pva/systematic_strike.json' if self.spline_setup else settings_path(self.root,self.method),self.cfg);self.setup_note.setText('Saved '+self.method.upper()+' setup. Existing runs and the other method are unchanged.');self.changed.emit()
    ...
    def refresh_models(self):
        ...
        if index<0:
            ...
            self.models.addItem('Reviewed M0 development · simulation only' if reviewed else 'Selected bootstrap model · fitting / unavailable' if selected else 'No fitted model selected',selected);index=self.models.count()-1
    ...
    def refresh_library(self):
        ...
        for i,p in enumerate(self.runs):
            ...
            if self.method=='mppi':
                ...
                values.append('Targeted strike · saved fold rule' if task.get('success_criterion')=='targeted_fold_strike_v1' else 'Tip reaches target' if task.get('success_criterion')=='tip_contact_v1' else
        ...
        if self.method=='mppi':selectors.append(self.live_view.run_selector)
        for selector in selectors:
            selector.blockSignals(True);selector.clear()
            for p in self.runs:
                identity=read_json(p/'identity.json',{});status=read_json(p/'status.json',{})
    ...
    def poll(self):
        ...
        if self.method=='mppi':
            ...
            selector.blockSignals(True);selector.setCurrentIndex(selector.findData(str(p)));selector.blockSignals(False)
        if not p:return
        ...
        if self.log_toggle.isChecked():
            log=p/'console.log'
            if not log.exists():log=p/'stdout.log'
            if log.exists():
        ...
        if self.method=='mppi':self.mppi_chart.setVisible(receding)
        ...
        if self.method=='mppi':
            ...
            task=read_json(p/'settings.json',{}).get('task',{})
            message=(('Fold diagnostic only; full offline targeted strike.' if read_json(p/'settings.json',{}).get('fold_requirement')=='diagnostic_only' else 'Travelling fold required before the scored strike; full offline trajectory.') if task.get('success_criterion')=='targeted_fold_strike_v1' else
                'Task requires forward pull, then backward drone motion at tip contact.' if requirement else 'Saved tip-hit task: backward release was not required.')
            self.progress_note.setText(self.progress_note.text()+'\n'+message)
        ...
        if not rows:
            self.figure.clear()
            if self.method=='mppi':
                ax=self.figure.subplots();ax.axis('off');ax.text(.5,.5,'Waiting for the first iteration result.' if search_view or not receding else 'Committed-path trends appear after the first command.',ha='center',va='center',wrap=True)
        ...
        for ax,key,title in zip(axes,keys,titles):
            factor=100 if key in ('success','failures') else 1;ax.plot(x,[(r.get(key) if r.get(key) is not None else float('nan'))*factor for r in rows],color='#2563eb',lw=1.6,label='Training batch' if self.method=='ppo' else None)
