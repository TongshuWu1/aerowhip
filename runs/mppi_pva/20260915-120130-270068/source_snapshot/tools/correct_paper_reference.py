"""Correct the original M0 command with M1 and export a fully replayed CSV."""
...
from planning.mppi_trajectory import adaptive_weights
...
def main():
    ...
    job=ROOT/'runs/reference_tracking'/('M1-'+stamp)
    ...
    samples=512;groups=4;iterations=38;seed=657
    cfg.update(model_path='model.json',method='mppi',reward={},
        ...
        correction=dict(original_spline_duration_s=cfg['task']['duration_s'],fixed_handover_s=meta['interval_s'][1],
            reference_sha256=meta['reference_sha256'],seed=seed,samples=samples,iterations=iterations,
            proposal_count=groups,position_noise_scales_m=[.005,.015,.04],target_ess_fraction=.2,
            initialization='Original M0 B-spline controls; no previous corrected command',
            weighting='Adaptive-temperature exponential costs, zero control prior',
            units='All tracking terms are mean squared 3D distances in m^2',
    ...
    sources={str(f):sha256_file(f) for f in [Path(__file__),ROOT/'planning/reference_correction.py',
        ROOT/'planning/position_spline.py',ROOT/'planning/mppi_trajectory.py',
    ...
    try:
        ...
        batch=CoupledRollout(engine,samples+groups+1,origin,initial_q,limits)
        ...
        best=baseline.clone();best_cost=float('inf');best_terms={}
        if bool(unchanged['complete_valid'][0]):
            try:finish(best);best_cost=float(base_cost[0]);best_terms={k:float(v[0]) for k,v in base_terms.items()}
            except ValueError as exc:print('Baseline recovery rejected:',exc,flush=True)
        means=baseline[None].expand(groups,-1,-1).clone()
        # The retained smooth covariance is derived from spline geometry only.
        basis=spline.scratch_noise_basis()
        rng=torch.Generator(device='cuda').manual_seed(seed);history=[]
        per=samples//groups;scales=tensor(cfg['correction']['position_noise_scales_m'])
        for iteration in range(1,iterations+1):
            if (job/'STOP').exists():raise InterruptedError('Correction stopped; no CSV exported')
            status('Correcting fixed tip trajectory',iteration=iteration,iterations=iterations,
                best_cost_m2=best_cost if np.isfinite(best_cost) else None)
            noise=torch.randn(groups,per,9,3,device='cuda',dtype=torch.float64,generator=rng)
            noise=torch.einsum('ij,gpjc->gpic',basis,noise)
            noise*=scales[torch.arange(per,device='cuda')%len(scales)][None,:,None,None]
            draws=means[:,None]+noise
            free=torch.cat((draws.reshape(samples,9,3),means,best[None]))
            packets,jerk=spline.decode(free,origin);packets=packets[:,:cutoff]
            valid=spline.jerk_valid(free,origin,tensor(cfg['action']['jerk_limit_m_s3']))
            valid &= command_valid(packets,limits)
            safe=torch.where(valid[:,None,None],packets,base_packets[None])
            prediction=batch(safe,times,grid)
            cost,terms=tracking_cost(prediction['cable_positions_m'][:,:,-1],
                prediction['position_origin_m'],packets,reference,weights)
            cost=cost.masked_fill(~valid|~prediction['complete_valid']|~torch.isfinite(cost),torch.inf)
            rejected=0
            for index in cost.argsort().tolist():
                value=float(cost[index])
                if not np.isfinite(value) or value>=best_cost:break
                try:finish(free[index])
                except ValueError:
                    cost[index]=torch.inf;rejected+=1;continue
                best=free[index].clone();best_cost=value
                best_terms={k:float(v[index]) for k,v in terms.items()};break
            # Fixed scaling only conditions the temperature search; it does
            # not change relative term weights or the accepted candidate order.
            w,temp,ess=adaptive_weights((-1000*cost[:samples]).reshape(groups,per),.2)
            updated=(w[:,:,None,None]*draws).sum(1)
            means=torch.where((w.sum(-1)>0)[:,None,None],updated,means)
            row=dict(iteration=iteration,cost_m2=best_cost if np.isfinite(best_cost) else None,
                terms_m2=best_terms,valid_fraction=float(torch.isfinite(cost[:samples]).double().mean()),
                recovery_rejections=rejected,effective_samples=float(ess.sum()),elapsed_s=time.perf_counter()-start)
            history.append(row);atomic_json(job/'history.json',history)
            np.savez_compressed(job/'current_best.npz',position_control_points_m=best.cpu().numpy())
        ...
        output=ROOT/'runs/rehearsals_pva'/f'{stamp}-M1-fixed-tip-reference'
        ...
        summary=dict(schema='pva_fullstate_30hz_v1',planner='MPPI fixed-reference correction',
            ...
            csv_sha256=sha256_file(output/'fullstate_30hz.csv'),iterations=iterations,
            random_candidate_budget=samples*iterations,elapsed_s=time.perf_counter()-start,
        ...
        export=ROOT/'exports/M1_fixed_tip_reference';export.mkdir(exist_ok=False)
        ...
        (export/'README.md').write_text(
            '# M1 fixed-tip-reference command\n\n'
