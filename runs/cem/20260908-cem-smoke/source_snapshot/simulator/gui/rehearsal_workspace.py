...
class RehearsalWorkspace(QWidget):
    ...
    def load_result(self,directory):
        ...
        if self.metadata.get('schema')!='research_fullstate_30hz_v1':raise ValueError('Select a completed native 30 Hz rehearsal folder.')
        ...
        self.status.setText(f'Policy {m["checkpoint_sha256"][:8]} · whip {m["whip_end_s"]:.2f} s · total CSV {m["total_duration_s"]:.2f} s · '
    ...
    def export_package(self):
        ...
        if path:
            ...
            try:export_package(self.directory,Path(path));self.status.setText('Policy, both model residuals, CSV and provenance exported: '+path)
