...
class IsaacLabTrainingSession:
    def __init__(self,app,artifact,batch,*,headless=False,render_stride=5,spacing=4.):
        ...
        self.drones.CreateProtoIndicesAttr([]);self.drones.CreatePositionsAttr([])
        ...
        self.targets.CreateProtoIndicesAttr([]);self.targets.CreatePositionsAttr([])
        ...
        floor=UsdGeom.Cube.Define(self.usd,'/World/DisplayFloor');floor.CreateSizeAttr(1.)
        floor.AddScaleOp().Set(Gf.Vec3d(max(12.,np.sqrt(batch)*spacing+8),max(12.,np.sqrt(batch)*spacing+8),.02))
        floor.AddTranslateOp().Set(Gf.Vec3d(0,0,-1.))
        floor.CreateDisplayColorAttr([(.16,.18,.22)])
    ...
    def end_collection(self,score,rollout):
        ...
        colors=np.tile([1.,.55,.1],(score.batch_size,1))
        colors[score.episode_success.detach().cpu().numpy()]=[.1,.8,.25]
        colors[score.failed.detach().cpu().numpy()]=[1.,.1,.1]
        self.colors.Set(self.Vt.Vec3fArray.FromNumpy(colors.astype(np.float32)))
    ...
    def close(self):
        from simulator.workflow import read_json
        status=read_json(self.artifact/'status.json',{})
        self.stage=status.get('status','Session closed')
