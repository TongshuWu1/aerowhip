...
from planning.cem_run import DEFAULTS,prepare_job,validate_settings
...
class CEMPage(QWidget):
    def __init__(self,root):
        ...
        launch_group=QGroupBox('CEM launch setup');launch_form=QFormLayout(launch_group)
        ...
        self.save_launch=QPushButton('Save CEM launch setup');self.save_launch.clicked.connect(self.save_launch_setup)
        launch_form.addRow(self.save_launch);form.addRow(launch_group)
        self.spins={}
        labels={'population':'Parallel candidates','iterations':'CEM iterations','elite_fraction':'Elite fraction','control_points':'Spline control points',
        ...
        self.settings_page=CEMSettingsPage();self.tabs.addTab(self.settings_page,'Task & rewards')
        ...
        saved=read_json(self.root/'config/cem.json',{})
    ...
    def set_running(self,running):
        ...
        for widget in [self.seed,self.name,self.device,self.save_launch,*self.spins.values(),*self.start_spins,*self.target_spins]:widget.setEnabled(not running)
    ...
    def settings_values(self):
        ...
        settings.update(display_name=self.name.text().strip() or 'CEM spline',device=self.device.currentText(),**self.settings_page.values())
        settings['launch_setup']=self.launch_values()
        validate_settings(dict(DEFAULTS,**settings))
    ...
    def save_launch_setup(self):
        ...
        try:
            settings=read_json(self.root/'config/cem.json',{})
            settings['launch_setup']=self.launch_values()
            atomic_json(self.root/'config/cem.json',settings)
            self.status.setText('Saved CEM launch setup for new plans. PPO settings are unchanged.')
        except (ValueError,OSError) as error:self.status.setText('Cannot save launch setup: '+str(error))

    def save_settings(self):
        from experimental_data.io import atomic_json
        try:
            settings=self.settings_values();atomic_json(self.root/'config/cem.json',settings)
            self.settings_page.status.setText('Saved CEM launch setup, reward, hit criteria and search settings for new runs. Existing results are unchanged.')
    ...
    def load_run_settings(self):
        ...
        if not path:self.settings_page.status.setText('Select a saved CEM run on Optimize spline or Rehearsal & Export first.');return
        try:
            settings=read_json(Path(path)/'cem.json');task=read_json(Path(path)/'task.json')
            ...
            self.apply_settings(dict(DEFAULTS,**settings));self._criteria_initialized=True
