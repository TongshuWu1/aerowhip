...
def prepare_job(root, seed_directory, output, settings, origin=None, target=None):
    ...
    default_origin,default_target=launch_positions(root,meta['initial_tracking_origin_m'],meta['target_position_m'])
    origin = np.asarray(default_origin if origin is None else origin, float)
    target = np.asarray(default_target if target is None else target, float)
