...
class SimulatorMainWindow(QMainWindow):
    def __init__(self, project_root, model_config, task_config, ppo_config):
        ...
        for page, (title, _) in zip((self.model_page, self.recordings_page, self.training_page,
                                    self.diagnostics_page, self.fullstate_page, self.cem_page), PAGE_DEFINITIONS):
    ...
    def closeEvent(self, event):
        self.closing = True
        ready = [page.shutdown() for page in (self.training_page, self.fullstate_page, self.cem_page)]
        if not all(ready):
            event.ignore()
            return
