    ...
    for report in processing['reports']:
        ...
        with np.load(folder/'dataset.npz',allow_pickle=False) as d:
            ...
            np.savez_compressed(output/(name+'.npz'),**{k:v for k,v in state.items() if isinstance(v,np.ndarray)},
                ...
                pose_history_time_s=t[hover],pose_history_position_origin_m=p[hover],
                pose_history_quaternion_xyzw=q[hover],pose_history_position_valid=d['drone_position_valid'][hover],
                pose_history_orientation_valid=d['drone_orientation_valid'][hover],
