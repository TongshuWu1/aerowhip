...
class SimulatorMainWindow(QMainWindow):
    ...
    def select_adapted_training_model(self,path):
        ...
        page.run_name.setText('M1 PPO adaptation - original flight setup')
    ...
    def _page_changed(self, index):
        ...
        if index==5:
            self.model_status.setText('M1 default · MPPI force actions → PPO virtual model → FullState → fitted drone/cable execution')
        else:
            self.model_status.setText('PPO workspace: M0 · 30 Hz · saved results and adaptation ghosts use their own frozen model')
    ...
    def closeEvent(self, event):
        self.closing = True
        ready = [page.shutdown() for page in (self.training_page, self.fullstate_page, self.mppi_page, self.adaptation_check_page)]
        if not all(ready):
            event.ignore()
