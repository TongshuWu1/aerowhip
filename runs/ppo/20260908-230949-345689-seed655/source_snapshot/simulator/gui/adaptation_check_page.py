...
class AdaptationCheckPage(QWidget):
    def __init__(self, root, parent=None):
        ...
        layout.addWidget(note('Solid orange: OptiTrack measurement · Blue ghost: saved predicted execution, not the commanded PVA. World XYZ in metres. Missing markers remain gaps.'))
    ...
    def loaded(self, data):
        ...
        self.status.setText(f"{data['take']} · {coverage} · tracking {start:.2f} to {end:.2f} s relative to CSV\n"
            f"Saved prediction: {data['rehearsal'].name} · exact CSV match · measurement alignment RMS {a['rms_m']*100:.2f} cm (not flight error). Timing includes logging latency.")
