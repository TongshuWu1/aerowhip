...
class AlgorithmTrainingPage(QWidget):
    ...
    def __init__(self, root, algorithm, parent=None):
        ...
        self.live_scene = QCheckBox('Open live Isaac scene for this training run (native 30 Hz)')
    ...
    def start_training(self, resume):
        try:
            ...
            directory, command = prepare_training(self.root, self.algorithm, seed=self.seed.value(),
                ...
                overrides=overrides, resume=checkpoint, run_name=self.run_name.text(),live_scene=self.live_scene.isChecked())
            ...
            if self.live_scene.isChecked():self.live_scene_requested.emit(str(directory))
