...
def initialize_proposal(actual,candidates,settings,horizon):
    ...
    bank=pullback_seed_bank(actual,horizon)
    ...
    details=dict(type='pullback_jerk_guess',candidate_count=len(indices),score=float(value),
...
def optimize(job,model,cfg):
    ...
    mean,initialization=initialize_proposal(actual,candidates,s,horizon)
