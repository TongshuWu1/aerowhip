...
def execute_research_batch(nominal,batch,forces,cutoffs,settings,*,trace=None,progress=None):
    ...
    seconds=torch.where(score.episode_success,score.episode_hit_time_s,cutoffs*dt)
    extra=score.reward_weights.time_per_s*(cutoffs*dt-seconds)
    score.episode_reward-=extra;score.episode_component_sums['time']-=extra
