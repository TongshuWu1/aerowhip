...
from .gentle_recovery import plan_recovery
...
def complete_packets(whip,hover,settings=None):
    ...
    sample,recovery=plan_recovery(whip[-1,:3],whip[-1,3:6],whip[-1,6:9],hover,settings)
...
def generate(checkpoint,output,origin,target,device='cuda',progress=None):
    ...
    with torch.no_grad():
        ...
        whip=score.reference_packets[0].cpu().numpy()
        ...
        prefix=len(frames)
        discrepancy=(pose['position_origin_m'][:,:prefix]-score.predicted_pose['position_origin_m']).abs().max().item()
        if discrepancy>1e-8:raise ValueError(f'Training/export prediction mismatch: {discrepancy:g} m')
        cable=list(frames);current=score.execution_state;valid_end=prefix-1
...
def export_package(directory,destination):
    ...
    model=read_json(directory/'model.json');model['motion_residual']['checkpoint']='assets/cable_residual.pt'
