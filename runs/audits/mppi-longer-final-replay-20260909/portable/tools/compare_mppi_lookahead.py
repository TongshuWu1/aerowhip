...
def main():
    ...
    for horizon in (1.2,2.):
        ...
        while True:
            ...
            score=r['reward'].masked_fill(r['failed'],-torch.inf)
            ...
            if float(value)>best:
                best=float(value);metrics=dict(reward=best,hit=bool(r['success'][index]),distance_m=float(r['minimum_tip_distance_m'][index]),duration_s=float(r['duration_s'][index]))
