"""Immutable direct-force MPPI jobs and independent FullState export."""
...
from .mppi_contract import rollout_contract,resolve_reward,resolve_action,LAUNCH_DEFAULTS

DEFAULTS=dict(optimizer='mppi',representation='independent_force_30hz_v2',population=128,iterations=12,
    temperature=20.,action_std=.2,horizon_s=2.,random_seed=655,device='cuda',
...
def validate_settings(settings):
    ...
    resolve_reward(settings.get('reward'));resolve_action(settings.get('action'))
    if 'ppo_reward' in settings:raise ValueError('Legacy PPO-coupled settings: use the independent MPPI reward fields.')


def prepare_job(root,output,settings,origin=None,target=None):
    ...
    launch=resolve_launch_setup(settings.get('launch_setup',LAUNCH_DEFAULTS))
    ...
    settings.update(representation='independent_force_30hz_v2',optimizer='mppi',model_path=str(model_path),
        success=task['success'],desired_strike_direction_world=task['desired_strike_direction_world'],
        reward=resolve_reward(settings.get('reward')),action=config['action'],initialization='hanging_hover')
    output.mkdir(parents=True,exist_ok=True)
    atomic_json(output/'model.json',snapshot_assets(model,output))
    for n,value in (('task',task),('rollout',config),('mppi',settings)):atomic_json(output/f'{n}.json',value)
    atomic_json(output/'model_provenance.json',dict(source=str(model_path),source_sha256=sha256_file(model_path)))
    atomic_json(output/'initialization.json',dict(method='hanging_hover',normalized_action=[0.,0.,0.],
        description='Independent MPPI Gaussian prior: exact hanging hover force at every 30 Hz tick.'))
    ...
    for name in ('requirements.txt',):shutil.copy2(root/name,snapshot/name)
...
def complete_prediction(evaluation,origin,model,config,settings):
    ...
    if bool(score.failed[0]) or not hasattr(score,'reference_packets'):raise ValueError('Model execution rejected this force sequence.')
    ...
    if not bool(valid.all()):raise ValueError('Complete FullState reference exceeds the model command envelope.')
    ...
    if error>1e-8:raise ValueError('Scoring/export prefix mismatch.')
    ...
    meta=dict(schema='mppi_force_fullstate_30hz_v1',planner='MPPI 30 Hz force sequence',optimizer='mppi',representation='independent_force_30hz_v2',
...
def run_job(output):
    ...
    model,task,config=[read_json(output/f'{n}.json') for n in ('model','task','rollout')]
    ...
    try:
        ...
        def progress(row,best,bank):
            ...
            atomic_json(output/'progress.json',dict(row,label=f'MPPI force iteration {row["iteration"]}/{settings["iterations"]} · MPPI objective {row["best_score"]:.2f}',step=row['iteration'],total=settings['iterations']))
        ...
        for candidate_index,(_,vector) in enumerate(bank):
            atomic_json(output/'progress.json',dict(label=f'Checking force candidate {candidate_index+1}/{len(bank)}: complete rehearsal and export limits',step=settings['iterations'],total=settings['iterations']))
            ...
            try:
                ...
                rejected.append(str(error));atomic_json(output/'recovery_rejections.json',rejected);continue
