...
def research_physics_step(physics,state,command,dt,constants,*,controller=None,create_graph=False,fast_solve=False,fast_geometry=False):
    ...
    return physics.step_runtime(state,boundary,dt,constants,
        ...
        create_graph=create_graph,iterative_damping=False,analytic_bending=True,
        dense_constraint_solve='cuda_tridiagonal' if fast_solve else True,vectorized_curvature=fast_geometry)


class ResearchPhysics:
    def __init__(self,physics,state,dt,*,controller=None,constants=None,graph=True,fast_solve=False,fast_geometry=False):
        self.fast_solve=fast_solve
        self.fast_geometry=fast_geometry
    ...
    def _step(self):
        return research_physics_step(self.physics,DderState(self.q,self.v),self.input,self.dt,self.constants,
            controller=self.controller,create_graph=False,fast_solve=self.fast_solve,fast_geometry=self.fast_geometry)
