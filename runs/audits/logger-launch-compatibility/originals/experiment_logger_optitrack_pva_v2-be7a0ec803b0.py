...
def parse_args(argv=None):
    ...
    parser.add_argument('--expected-rate', type=float, default=100., help='Expected TF rate for gap diagnostics; does not change the publisher rate.')
...
def main():
    ...
    if output.suffix.lower() != '.csv':
        output /= 'experiment_' + datetime.now().strftime('%Y%m%d_%H%M%S_%f') + '.csv'
    rclpy.init(args=ros_args)
    node = Node('optitrack_pva_logger_v2')
    recorder = None
    status = 'complete'
    try:
