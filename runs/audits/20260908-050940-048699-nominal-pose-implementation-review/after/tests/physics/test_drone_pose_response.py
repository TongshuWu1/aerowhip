...
def test_multi_axis_pose_matches_independent_adaptive_ode():
    ...
    def ode(t,y):
        ...
        a=4*(values[:3]-p)+3*(values[3:6]-v)+values[6:9]+comp
        z=a+np.array([0,0,9.80665]);z/=np.linalg.norm(z)
    ...
    for step in [.01,.005,.0025]:
        out=predict_pose(state,stream,[0,.4],parameters(delay_s=0.),offset_tracking_m=[.007,-.014,-.055],maximum_step_s=step)
