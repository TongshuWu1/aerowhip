...
class MultiDronePage(QWidget):
    def __init__(self, root):
        ...
        follow=QPushButton('Open live training scene');follow.clicked.connect(lambda:self.follow_training(self.live_runs.currentData()));live_row.addWidget(follow);layout.addLayout(live_row)
    ...
    def follow_training(self,run):
        if not run:self.status.setText('Start a new native 30 Hz policy with the live Isaac option enabled.');return
