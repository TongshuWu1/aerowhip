...
ENCOUNTER_FIELDS = ('encounter_time','encounter_distance','encounter_q',
    ...
    'encounter_backward','encounter_tip_first','hit_tip_velocity')


def record_encounter(env, previous, previous_origin, previous_velocity, q, v,
                     entry, nearest_fraction, running, clock_left,previous_pull_peak):
...
"""
    ...
    forward=((origin-env.origin0)*env.direction).sum(-1)
    values=dict(encounter_time=clock_left+f*env.dt,encounter_distance=distance,encounter_q=cq,
        ...
        encounter_backward=torch.maximum(previous_pull_peak,forward)-forward,
...
class PreferredFoldCapture(ObjectiveCapture):
    ...
    def score(self,env,result,actions,weights):
        ...
        from learning.two_target_whip import enabled,score_terms,completion_terms
        if enabled(env.settings):
            terms.update(score_terms(env,weights));terms=completion_terms(env,weights,terms)
