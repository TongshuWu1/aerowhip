"""Independently check serialized selected-M2 commands, model identity and UI loading."""
...
def main():
    ...
    m=json.loads((rehearsal/'model.json').read_text())
    dp=rehearsal/m['fullstate_execution']['checkpoint']
    d=json.loads(dp.read_text())
    payload=torch.load(dp.parent/d['residual']['checkpoint'],map_location='cpu',weights_only=True)
    assert all(torch.count_nonzero(v)==0 for v in payload['state_dict'].values())
    assert m['motion_residual']['enabled']
    ...
    library=PVAModelPage(ROOT)
    try:
        index=library.models.findData(str((source/'candidate/model.json').resolve()))
        assert index>=0
        library.models.setCurrentIndex(index);library.inspect()
        values={library.table.item(i,0).text():library.table.item(i,1).text() for i in range(library.table.rowCount())}
        assert values['Drone residual']=='Disabled'
    finally:library.shutdown();library.close();app.processEvents()
    receipt=dict(csv_sha256=summary['csv_sha256'],rows=len(rows),rate_hz=30,duration_s=float(t[-1]),
        cable_only_M2_identity_unchanged=True,drone_residual_exactly_zero=True,cable_residual_enabled=True,original_reference_hash_verified=True,
        ...
        start_end_settled=True,command_envelope_passed=True,offscreen_UI_loaded=True,model_library_shows_drone_residual_disabled=True,
