...
def build():
    ...
    for idx in IDS[1:]:
        ...
        Image.fromarray(np.ceil(alpha*255).astype(np.uint8)).save(OUT / f'mask_{idx}.png')
    ...
    # Real target inset, enlarged only; no generated hardware.
    ...
    final.save(OUT / 'fig1_photographic_composite.png', dpi=(600,600))
    final.thumbnail((1400,1400))
    final.save(OUT / 'fig1_photographic_preview.png')
    ...
    print(OUT / 'fig1_photographic_composite.png')

if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--inspect', action='store_true')
    args = parser.parse_args()
    inspect() if args.inspect else build()
