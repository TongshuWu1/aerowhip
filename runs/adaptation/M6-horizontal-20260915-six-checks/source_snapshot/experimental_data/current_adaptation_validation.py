...
def summarize_prediction(trial,times,pose,cable):
    ...
    arrays=dict(time=times,position=predicted_p,rotation=predicted_r,cable=predicted_q,measured_position=p,measured_sites=sites)
    # Statistics use independent native 100 Hz observations, not an inflated
    # count of interpolated 150 Hz measurement samples.
    native=trial.data['time']
    scored=native[(native>=times[0])&(native<=times[-1])]
    predicted_p=interpolate_positions(times,predicted_p,scored)
    predicted_q=interpolate_positions(times,predicted_q,scored)
    from scipy.spatial.transform import Slerp
    predicted_r=Slerp(times,Rotation.from_matrix(predicted_r))(scored).as_matrix()
    times=scored;p,r,sites=trial.measured(times)
    ...
    strike=.94;pred_at=interpolate_positions(arrays['time'],arrays['cable'][:,-1],np.array([strike]))[0]
    measured_at=trial.measured(np.array([strike]))[2][0,-1]
    result=dict(scoring='native_100hz_observations_v1',observed_grid_span_s=[times[0],times[-1]],origin=metric(predicted_p,p,times),
...
def evaluate_model(job,trial,model,folder):
    ...
    if (folder/'metrics.json').exists():
        old=read(folder/'metrics.json')
        if old.get('scoring')=='native_100hz_observations_v1':return old
        with np.load(folder/'prediction.npz') as z:
            pose={k:torch.tensor(z[v][None]) for k,v in [('position_origin_m','position'),('rotation_tracking_to_world','rotation')]}
            metrics,_=summarize_prediction(trial,z['time'],pose,torch.tensor(z['cable'][None]))
        for key in ['initial_projection_max_coordinate_m','all_pose_domains_valid']:metrics[key]=old[key]
        save(folder/'metrics.json',metrics);return metrics
