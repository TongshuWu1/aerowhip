...
def note(job,stage,**info):
    if (Path(job)/'STOP').exists():
        raise InterruptedError('Adaptation stopped at a fit/update boundary; partial results preserved.')
...
def fit_nominal(trials,prior,folder,job,*,cold_start=False):
    ...
    if cold_start:
        low=np.array([.1,.1,.1,.1,.05,.05]);high=np.array([80.,80.,20.,20.,3.,3.])
    choices=[]
    for delay in sorted(set(([0.,.02,.04,.06,.08,.10,.12] if cold_start else [.02,.04,.06,.08,.10])+[prior.delay_s])):
        cache={}
        def objective(g):
            if 'g' not in cache or not np.array_equal(g,cache['g']):
                ...
                for t in trials:
                    ...
                    delta=np.where(w[:,None]>0,p-t.truth['position_origin_m'],0.)
                    errors.append((delta*w[:,None]).ravel())
        ...
        fit=least_squares(lambda x:objective(x)[0],gains0,jac=lambda x:objective(x)[1],bounds=(low,high),
            max_nfev=150 if cold_start else 45,ftol=1e-7,xtol=1e-7,gtol=1e-7,x_scale=gains0)
    ...
    if cold_start:lower=np.array([.1,.05,.02]);upper=np.array([3.,1.5,.30])
    traces=[]
    def attitude(x):
        values=np.exp(x);errors=[]
        try:
            for tr,c in zip(trials,cached):
                p=c.predict(values);valid=tr.weights>0
                err=Rotation.from_matrix(p[valid].transpose(0,2,1)@c.truth[valid]).as_rotvec()
                errors.append((err*np.sqrt(tr.weights[valid,None]/len(trials))/.15).ravel())
        ...
        return out
    a=least_squares(attitude,np.log(initial),bounds=(np.log(lower),np.log(upper)),max_nfev=100 if cold_start else 30,diff_step=1e-4,
    ...
    save(folder/'nominal_fit.json',dict(parameters=asdict(params),translation_profiles=choices,
        ...
        training=[t.name for t in trials],geometry_frozen=True,
        prior='explicit cold nominal estimates; no learned prior' if cold_start else 'M0 nominal; old NN omitted for nominal fit',
        translation_boundary_parameters=[GAIN_NAMES[i] for i in range(6) if min((g[i]-low[i])/(high[i]-low[i]),(high[i]-g[i])/(high[i]-low[i]))<.001],
        attitude_success=bool(a.success)))
    return params
...
class WeightedTranslationBatch(TranslationBatch):
    def loss(self,network,regularization=.01):
        ...
        for p,y,t in zip(predictions,self.truth,self.trials):
            ...
            target=torch.where(w[:,None]>0,y,p.detach())
...
def cable_run(job=JOB,*,grid_only=False):
