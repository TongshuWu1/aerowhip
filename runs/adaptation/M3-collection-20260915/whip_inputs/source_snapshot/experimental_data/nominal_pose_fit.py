...
def parameters(gains=PRIOR,tau=.08,delay=.02,attitude_scales=(1.,1.)):
    return PoseResponseParameters(*map(float,gains),float(tau),float(delay),attitude_drive_model='independent_scale_v3',
        attitude_acceleration_scale_xy=attitude_scales[0],attitude_acceleration_scale_z=attitude_scales[1])
...
class PreparedTrial:
    def __init__(self,folder,*,review=None,exclusions=()):
        ...
        zero=np.zeros(6);zero[4:]=1.;self.b0=self.state(zero,'cpu').compensation[0].numpy()
    ...
    @torch.no_grad()
    def pose(self,gains,tau,delay,*,device='cuda',step=.005,maneuver_only=False,attitude_scales=(1.,1.)):
        if device=='cuda':
        ...
        times=self.time[self.time<=self.context['csv_end_s']] if maneuver_only else self.time
        return predict_pose(self.state(gains,device,tau,delay,attitude_scales),stream,times,parameters(gains,tau,delay,attitude_scales),offset_tracking_m=self.offset,maximum_step_s=step)
...
def fit_attitude(trials,gains,delay,*,device='cuda',progress=print):
    ...
    def objective(log_tau):
        ...
        try:
            for trial in trials:
                out=trial.pose(gains,tau,delay,device=device,maneuver_only=True)
                count=out['rotation_tracking_to_world'].shape[1]
                mask=trial.masks['fit_orientation'][:count]
                predicted=out['rotation_tracking_to_world'][0,mask]
                actual=torch.as_tensor(trial.truth['rotation_tracking_to_world'][:count][mask],dtype=predicted.dtype,device=predicted.device)
                loss+=float(rotation_log(predicted.transpose(-1,-2)@actual).square().sum(-1).mean().cpu())/len(trials)
        except ValueError as exc:
            # Reject only this explicit candidate-domain failure. Missing data,
            # interface mistakes and other errors must still stop the job.
            if str(exc)!='Attitude error is too close to 180 degrees for this local response model':
                raise
            trace.append(dict(tau_s=tau,mean_squared_angle_rad2=None,valid=False,reason=str(exc),take=trial.name))
            progress(f'Attitude candidate {tau:.4f} s rejected: {exc}',flush=True)
            return np.inf
        if not np.isfinite(loss):
            raise FloatingPointError('Non-finite attitude fitting loss')
        trace.append(dict(tau_s=tau,mean_squared_angle_rad2=loss,valid=True))
        if len(trace)%5==0:progress(f'Attitude evaluation {len(trace)}: tau {tau:.4f} s, RMS {np.degrees(np.sqrt(loss)):.2f} deg',flush=True)
        return loss
    # A local-domain rejection can make the objective discontinuous. Establish
    # finite regions on a fixed log grid before refining the best grid cell.
    grid=np.geomspace(.02,.30,11)
    for tau in grid:objective(np.log(tau))
    valid=[row for row in trace if row['valid']]
    if not valid:
        return dict(valid=False,tau_s=None,mean_squared_angle_rad2=None,trace=trace,optimizer_success=False,
            bounds_s=[.02,.30],near_bound=None,reason='No valid attitude candidate on the fixed search grid')
    best=min(valid,key=lambda row:row['mean_squared_angle_rad2'])
    i=int(np.argmin(abs(grid-best['tau_s'])))
    result=minimize_scalar(objective,bounds=(np.log(grid[max(0,i-1)]),np.log(grid[min(len(grid)-1,i+1)])),
        method='bounded',options=dict(xatol=.002,maxiter=30))
    valid=[row for row in trace if row['valid']]
    best=min(valid,key=lambda row:row['mean_squared_angle_rad2'])
...
def diagnostic_pose(trial,gains,tau,delay,*,attitude_scales=(1.,1.)):
    """Record post-hold domain failure without inventing recovery predictions.

    A maneuver-domain failure still aborts assessment. A post-hold failure is
    explicitly reported and that entire diagnostic segment is unavailable.
    """
    try:
        return trial.pose(gains,tau,delay,attitude_scales=attitude_scales),dict(status='FULL_DIAGNOSTIC_PREDICTED',maneuver_only=False)
    except ValueError as exc:
        if str(exc)!='Attitude error is too close to 180 degrees for this local response model':raise
        result=trial.pose(gains,tau,delay,maneuver_only=True,attitude_scales=attitude_scales)
        count=result['rotation_tracking_to_world'].shape[1]
        padded={}
        for key,value in result.items():
            arr=torch.full((value.shape[0],len(trial.time),*value.shape[2:]),torch.nan,dtype=value.dtype,device=value.device)
            arr[:,:count]=value;padded[key]=arr
        return padded,dict(status='POST_HOLD_DOMAIN_FAILURE',maneuver_only=True,reason=str(exc),
            last_assessed_time_s=float(trial.time[count-1]),unavailable_samples=len(trial.time)-count)


def score_pose(trial,result):
    arrays={key:value[0].detach().cpu().numpy() for key,value in result.items()}
    angle=np.full(len(trial.time),np.nan)
    predicted_rotation_valid=np.isfinite(arrays['rotation_tracking_to_world']).all(axis=(1,2))
    valid=trial.masks['score_orientation']&predicted_rotation_valid
    if valid.any():
    ...
    for phase in ['csv_maneuver','post_maneuver_fullstate_hold']:
        ...
        for point in ['origin','attachment']:
            ...
            scores[phase][point+'_measured_samples']=int(mask.sum())
            mask&=np.isfinite(arrays[f'position_{point}_m']).all(axis=1)
        ...
        mask=active&trial.masks['score_orientation']&predicted_rotation_valid
        scores[phase]['orientation_samples']=int(mask.sum())
...
def translation_only_diagnostic(trial,gains,delay,reason):
    """Report a rejected pose candidate's translation, with no fabricated R/A."""
    p,v,_=trial.linear(gains,delay);n=len(trial.time)
    arrays=dict(position_origin_m=p,velocity_origin_m_s=v,position_attachment_m=np.full((n,3),np.nan),
        rotation_tracking_to_world=np.full((n,3,3),np.nan))
    arrays,scores=score_pose(trial,{k:torch.as_tensor(a[None]) for k,a in arrays.items()})
    fine,_,_=trial.linear(gains,delay,step=.0025)
    convergence=dict(position_max_difference_m=float(np.linalg.norm(p-fine,axis=1).max()),
        attachment_max_difference_m=None,orientation_max_difference_deg=None)
    availability=dict(status='POSE_CANDIDATE_REJECTED',reason=reason,translation_only=True)
    return arrays,scores,availability,convergence
