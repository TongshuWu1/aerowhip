...
def report(job=JOB):
    ...
    assessment=dict(whip_improved=bool(result['recommend_for_new_planning']),
        ...
        reason='Whip-focused candidate retained separately; recovery is assessed separately and prospective validation remains pending',
    ...
    lines+=['','RMS is the square root of the mean squared 3D Euclidean position error, evaluated at native 100 Hz observation times. The table is the arithmetic mean of the five per-flight values. The prediction starts from causal measured pre-flight state for both models; the original saved deployment forecast is retained separately.','',
        ...
        '',f'Late recovery/hold drone RMS: {cm(assessment["baseline_late_drone_rmse_m"])} → {cm(assessment["adapted_late_drone_rmse_m"])} cm. Whip attitude RMS: {assessment["baseline_whip_attitude_rmse_deg"]:.2f} → {assessment["adapted_whip_attitude_rmse_deg"]:.2f} degrees. M1 is saved as a separate whip-focused candidate; the active default was not replaced.',
        ...
        'The fitted parameters describe local effective response, not globally identified vehicle or cable material properties. Effective command delay is confounded with clock-alignment uncertainty. The extended NN can inject energy; its bound does not guarantee stability on other commands. Inspect individual fold results for parameter bounds and selected delays.','',
        'The NN updates used a fixed conservative budget; this is not a convergence claim. Measured initial hover can still differ from nominal launch after a shared batch correction. Initial positions and attitude/ablation results are in `../assessment.json`; a future plan must distinguish commanded launch coordinates from measured initial state.', '',
        'The final attitude cache uses the native subdivision rule. Numerical checks are recorded separately in `../verification/`.','',
    ...
    if protocol.get('vertical_calibration'):
        bias=protocol['vertical_calibration']['bias_z_m']
        lines[2]=('Fresh M1 from M0 and the five cf7 adp0 recordings. All measurements and errors use hover-normalized Z '
            f'(measured Z minus {bias:.8f} m), applied once to drone and cable. Commands and the saved deployment forecast remain unchanged. '
            'Maneuver fitting excludes each held-out flight, but calibration uses pre/post holds from all five flights. '
            'These are conditional retrospective diagnostics, not fully independent validation or evidence of improved real flight. '
            'The all-five model is for subsequent planning; new flights are required.')
        lines=[line.replace('Measured drone streams establish the fixed time alignment;',
            'Previously audited, source-bound clock offsets establish time alignment;').replace(
            'data/model_candidates/20260908-adp0-M1/model.json',f'data/model_candidates/{job.name}/model.json') for line in lines]
...
def publish(job=JOB):
    ...
    folder=ROOT/'data/model_candidates'/job.name
    ...
    model['adaptation'].update(model_selected=False,
        ...
        limitation='Fixed-budget local adaptation; shared all-flight hover calibration; no convergence or new-flight improvement claim')
    protocol=read(job/'protocol.json')
    if protocol.get('vertical_calibration'):
        model['adaptation'].update(evaluation_frame='hover_normalized_z',
            vertical_calibration=protocol['vertical_calibration'],
            validation=protocol['heldout']+' All-five candidate evaluation is in-sample.',
            fitted_vehicle='cf7; source mass preserved. Replacement cf3 hardware observation is not the fitted vehicle.')
