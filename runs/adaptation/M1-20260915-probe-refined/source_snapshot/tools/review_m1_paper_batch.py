"""Reproduce the 2026-09-13 M1 intake and frozen-forecast evaluation.
...
EXPORT = ROOT / 'exports/M1_local_fixed_tip_reference'
BATCH = ROOT / 'data/flight_batches/M1_paper_local_correction_20260913'
OUT = ROOT / 'runs/data_review/M1-paper-20260913'
REHEARSAL = ROOT / 'runs/rehearsals_pva/20260913-032256-816506-M1-local-fixed-tip-reference'
ROLES = {f'M1_{i:03d}': ('adaptation' if i in (1, 2, 4) else 'validation') for i in range(1, 6)}
...
def main():
    ...
    protocol = dict(schema=adaptation.SCHEMA, model_id='M1', rehearsal=str(REHEARSAL), frozen_hashes=frozen,
        parent_generation=1, parent_forecast_model_sha256=sha256_file(REHEARSAL / 'model.json'),
        ...
        model_update='Existing staged full identification: new whip adaptation takes plus preliminary training; earlier M0 adaptation replay retained for M2',
        ...
        operator_review='User confirms all five are clean takes with no physical contact, abort or intervention; same M1 setup.',
