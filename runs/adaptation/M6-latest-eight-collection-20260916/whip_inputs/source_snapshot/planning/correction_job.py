"""Export a verified local correction with explicit model and output paths."""
...
@torch.no_grad()
def run(source,refdir,job,output,export,*,previous_rehearsal=None,iterations=30,strike_guard="none",strike_tolerance_m=0.,replay_controls_from=None):
    torch.set_num_threads(4)
    source,refdir,job,output,export=[Path(v).resolve() for v in (source,refdir,job,output,export)]
    if any(p.exists() for p in (job,output,export)):raise ValueError('All output directories must be new')
    if len({job,output,export})!=3 or any(a in b.parents for a in (job,output,export) for b in (job,output,export) if a!=b):raise ValueError('Output directories must be distinct and non-nested')
    if iterations<1 or strike_guard not in ('none','reference','target') or strike_tolerance_m<0:raise ValueError('Invalid correction settings')
    result=read_json(source/'fit/result.json')
    if result['status']!='completed':raise ValueError('Completed model fit required')
    ...
    local_settings=dict(maximum_iterations=iterations,initial_trust_radius=.5,maximum_trust_radius=1.,
        finite_difference_step=.01,maximum_derivative_refinements=3,maximum_jacobian_relative_difference=.05,damping=1e-4,
        relative_improvement_stop=.001,small_improvement_patience=3,optimality_tolerance=1e-5,
        minimum_trust_radius=1e-4,planned_strike_time_s=meta['planned_strike_time_s'],
        target_position_m=meta['physical_target_m'],strike_guard=strike_guard,strike_tolerance_m=strike_tolerance_m,
        backtracking_scales=[1.,.5,.25,.125,.0625,.03125,.015625])
    cfg.update(model_path='model.json',method='local_reference_correction',reward={},
        ...
        correction=dict(original_spline_duration_s=cfg['task']['duration_s'],fixed_handover_s=meta['interval_s'][1],
            ...
            initialization=str(previous_rehearsal) if previous_rehearsal else 'Original executed reference controls',
    ...
    sources={str(f):sha256_file(f) for f in [Path(__file__),ROOT/'planning/reference_correction.py',
        ROOT/'planning/jerk_reference_correction.py',
    ...
    try:
        ...
        jerk_mode='original_jerk_m_s3' in ref
        if jerk_mode:
            from planning.jerk_reference_correction import JerkCorrection
            spline=JerkCorrection(ref['original_command_packets'][0],ref['original_jerk_m_s3'],device='cuda')
            baseline=torch.zeros((9,3),device='cuda',dtype=torch.float64)
        else:
        ...
        if previous_rehearsal is not None:
            previous=Path(previous_rehearsal).resolve()
            previous_settings=read_json(previous/'settings.json')
            if previous_settings.get('correction',{}).get('reference_sha256')!=meta['reference_sha256']:raise ValueError('Previous correction uses another fixed reference')
            with np.load(previous/'plan.npz') as z:
                baseline=tensor(z[getattr(spline,'control_artifact_key','position_control_points_m')])
            for f in (previous/'plan.npz',previous/'settings.json'):sources[str(f)]=sha256_file(f)
            atomic_json(job/'source_hashes.json',sources)
        cutoff=len(ref['command_time_s'])
        base_packets,base_jerk=spline.decode(baseline,origin);base_packets=base_packets[:cutoff]
        if previous_rehearsal is None and not np.allclose(base_packets.cpu(),ref['original_command_packets'],atol=1e-10,rtol=0):
        ...
        single=CoupledRollout(engine,1,origin,initial_q,limits,production=replay_controls_from is not None)
        def finish(free):
        ...
        status('Checking unchanged reference command under the frozen candidate model')
        ...
        if replay_controls_from is None:
            ...
            from planning.local_reference_correction import strike_metrics,strike_allowed
            old=Path(replay_controls_from).resolve()
            old_settings=read_json(old/'settings.json')
            if read_json(old/'status.json')['status']!='failed':raise ValueError('Expected preserved failed export')
            if old_settings['correction']['reference_sha256']!=meta['reference_sha256']:
                raise ValueError('Saved correction uses another reference')
            if old_settings['trajectory_objective']!=cfg['trajectory_objective'] or old_settings['limits']!=limits:
                raise ValueError('Saved correction objective or limits changed')
            if model_identity(old/'model.json')[0]!=model_identity(job/'model.json')[0]:
                raise ValueError('Saved correction uses another frozen model')
            key=getattr(spline,'control_artifact_key','position_control_points_m')
            with np.load(old/'current_best.npz') as z:best=tensor(z[key])
            if not bool(spline.jerk_valid(best,origin,tensor(cfg['action']['jerk_limit_m_s3']))):
                raise ValueError('Saved controls violate unchanged jerk bounds')
            _,complete,_,_,_,replayed=finish(best)
            value,terms=tracking_cost(replayed['cable_positions_m'][:,:len(grid),-1],
                replayed['position_origin_m'][:,:len(grid)],tensor(complete[:cutoff])[None],reference,weights)
            best_cost=float(value[0]);best_terms={k:float(v[0]) for k,v in terms.items()}
            baseline_strike=strike_metrics(unchanged['cable_positions_m'][0,:,-1].cpu().numpy(),grid,
                ref['tip_position_m'],meta['planned_strike_time_s'],meta['physical_target_m'])
            final_strike=strike_metrics(replayed['cable_positions_m'][0,:len(grid),-1].cpu().numpy(),grid,
                ref['tip_position_m'],meta['planned_strike_time_s'],meta['physical_target_m'])
            if not strike_allowed(final_strike,baseline_strike,strike_guard,strike_tolerance_m):
                raise ValueError('Production replay fails the fixed-time strike guard')
            optimization=read_json(old/'optimization.json')
            optimization.update(reused_controls_from=str(old),final_replay_backend='standard production',
                baseline_strike=baseline_strike,final_strike=final_strike,
                controls_reoptimized=False,production_tracking_cost_m2=best_cost)
            atomic_json(job/'optimization.json',optimization)
            atomic_json(job/'replay_provenance.json',dict(original_failed_check=read_json(old/'status.json'),
                source=str(old),original_source_hashes=read_json(old/'source_hashes.json'),
                note='No optimization or tolerance relaxation; unchanged saved controls checked with standard production propagation.'))
            for f in (old/'current_best.npz',old/'optimization.json',old/'settings.json',old/'model.json',old/'status.json'):
        ...
        if difference>1e-5 or pose_difference>1e-9:
            raise ValueError(f'Independent complete replay differs: cable={difference}, pose={pose_difference}')
        ...
        representation=(dict(jerk_correction_coefficients_m_s3=best.cpu().numpy(),
                normalized_jerk=jerk.cpu().numpy()/np.asarray(cfg['action']['jerk_limit_m_s3'])) if jerk_mode else
            dict(position_control_points_m=best.cpu().numpy(),spline_knots_s=spline.knots,spline_degree=spline.degree))
        np.savez_compressed(job/'plan.npz',**representation,
            command_packets=decoded.cpu().numpy(),plan_complete=True,command_contract=cfg['command_contract'],
        ...
        summary=dict(schema='pva_fullstate_30hz_v1',planner='Local fixed-reference command correction',
            ...
            **({"standard_complete_cable_max_difference_m":difference,"standard_complete_pose_max_difference_m":pose_difference}
               if replay_controls_from is not None else
            ...
            evidence='Simulation-only command correction toward the frozen reference motion and timing; physical improvement has not been established.',
            reference_label=meta.get('reference_label','M0'),command_parameterization='additive_jerk_correction' if jerk_mode else 'position_spline',
        ...
        atomic_json(export/'export.json',dict(rehearsal=str(output),
            correction_job=str(job),**summary))
        (export/'README.md').write_text(
            '# Local fixed-tip-reference command\n\n'
            'Use `fullstate_30hz.csv` with the existing 30 Hz FullState flight program.\n'
            f'Launch tracked origin: {tuple(origin)} m. Reference strike point: {tuple(cfg["launch"]["target_m"])} m.\n'
            ...
            'Commands are corrected with the frozen candidate model to track the unchanged reference tip\n'
