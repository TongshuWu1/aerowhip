...
class CEMSettingsPage(QWidget):
    ...
    def __init__(self,planner='CEM'):
        ...
        layout.addWidget(note('Settings apply to the next CEM run. Save to keep them after restarting the app. Each run freezes its own launch setup, reward, hit criteria and search settings; PPO and existing results keep theirs.'))
        buttons=QHBoxLayout()
        ...
        if planner!='CEM':
            ...
            for widget in self.findChildren(QLabel)+self.findChildren(QPushButton):
                widget.setText(widget.text().replace('CEM',planner).replace('elite fraction','temperature'))
