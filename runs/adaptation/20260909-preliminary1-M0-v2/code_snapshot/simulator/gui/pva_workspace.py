...
class PVAPlannerPage(QWidget):
    ...
    def __init__(self,root,method):
        ...
        self.saved_log=QPlainTextEdit();self.saved_log.setReadOnly(True);self.saved_log.setMaximumHeight(130);self.saved_log.hide()
    ...
    def describe_model(self):
        ...
        self.model_note.setText((f'{provenance.get("label",self.models.currentText())}\nDrone {mass.get("drone_mass_kg",0)*1000:g} g · cable {mass.get("cable_assembly_mass_kg",0)*1000:g} g\n'+
            ('Fitted model; next flights provide prospective evidence.' if provenance.get('fit_complete') else 'Unfinished or historical model; inspect its original provenance.')) if m else 'No fitted model available. Collect and review the new preliminary takes before fitting M0 and planning.')
    ...
    def poll(self):
        ...
        if self.log_toggle.isChecked():
            ...
            if log.exists():
                with log.open('rb') as stream:
                    stream.seek(max(0,log.stat().st_size-14000));self.saved_log.setPlainText(stream.read().decode('utf-8',errors='replace'))
        ...
        if not rows:self.figure.clear();self.canvas.draw_idle();return
        ...
        if receding:
            ...
            keys=['actual_reward','actual_success','actual_minimum_tip_distance_m','iterations']
            titles=['Simulated return','Simulated valid hit','Closest simulated tip distance','Iterations per lookahead']
        for ax,key,title in zip(axes,keys,titles):
            ...
            ax.set_title(title,loc='left',fontsize=10);ax.set_xlabel('Simulated maneuver time [s]' if receding else 'Attempts' if self.method=='ppo' else 'Iterations');ax.grid(alpha=.2);ax.spines[['top','right']].set_visible(False)
