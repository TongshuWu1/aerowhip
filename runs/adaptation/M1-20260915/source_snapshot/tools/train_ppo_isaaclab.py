...
def main():
    ...
    app_launcher=AppLauncher(args,fast_shutdown=True);app=app_launcher.app
    ...
    try:
        ...
        status=read_json(args.artifact_directory/'status.json',{})
        status.update(status='FAILED',stage=str(error))
        atomic_json(args.artifact_directory/'status.json',status)
        # Kit fast shutdown may terminate Python before a pending exception is
        # printed. Publish the traceback and failure exit code before closing.
        import traceback
        import omni.kit.app
