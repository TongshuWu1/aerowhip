...
def test_training_viewport_executes_once_then_remains_in_pid(configs):
    from PySide6.QtWidgets import QApplication
    from simulator.gui.training_workspace import PolicyViewport
    from types import SimpleNamespace
    from threading import Event
    app = QApplication.instance() or QApplication([])
    page = PolicyViewport(ROOT, 'PPO')
    page.set_active(True)
    flight = LiveFlight(*configs, policy=lambda obs: torch.zeros((1, 3)))
    flight.settled_s = .5
    frame = flight.snapshot()
    commands = []
    pending=Event()
    def command(value):
        commands.append(value)
        pending.set()
    page.worker = SimpleNamespace(take_frame=lambda: {**frame, 'realtime_rate': 1.},
                                  command=command,strike_pending=pending,stop_requested=Event())
    page.tick()
    assert commands==[] and page.execute.isEnabled()
    page.execute.click()
    page.tick()  # A stale ready frame must not permit a duplicate request.
    page.execute.click()
    assert commands == ['strike']
    frame.update(phase=flight.POLICY, ready=False)
    page.tick()
    assert 'Open-loop strike' in page.status.text()
    frame.update(phase=flight.RECOVER, success=True)
    page.tick()
    assert 'PID recovery' in page.status.text()
    frame.update(phase=flight.HOVER, ready=True)
    pending.clear()
    page.tick()
    page.tick()
    assert commands == ['strike']
