...
def fixture(root):
    ...
    flights=[]
    for n in ('M0','M1'):
        batch=root/'rehearsal_csv_and_result_in_real_flight'/n
...
def test_generation_filter_keeps_identical_commands_different_forecasts_separate(tmp_path):
    ...
    p=tmp_path/'rehearsal_csv_and_result_in_real_flight/M1/protocol.json'
...
def test_select_model_clears_old_replay_and_future_is_empty(tmp_path):
    ...
    assert page.select_flight(tmp_path/'rehearsal_csv_and_result_in_real_flight/M0','whip_002')
...
def test_future_generation_inbox_groups_before_flight_report_registration(tmp_path):
    ...
    fixture(tmp_path)
    batch=tmp_path/'rehearsal_csv_and_result_in_real_flight/M1'
