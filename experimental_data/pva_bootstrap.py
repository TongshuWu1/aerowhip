...
def prepare_job(job,batch=None):
    ...
    protocol.update(schema='normalized_adp0_cold_pva_bootstrap_v1',seed=20260909,
        ...
        performance=dict(drone='full_cuda_loss_vjp_v1',cable='recomputed_cuda_vjp_blocks_v1',
            cable_dtype='float64',cable_graph_block_steps=3,batched_vertices=True,constraint_solver='cuda_tridiagonal'),
...
def fit_drone(job,trials,engine,settings):
    ...
    net=engine.drone.residual.to('cuda');batch=CudaDroneFit(trials,params,net)
    optimizer=torch.optim.Adam(net.parameters(),lr=.001,weight_decay=1e-4)
    def select():
        with torch.no_grad():return float(batch())
    result=selected_training(net,optimizer,batch,select,settings['drone'],folder,job,'drone residual')
    net.requires_grad_(False)
    # SciPy attitude refinement operates on its small CPU caches.
    del batch
    net.cpu()
...
def fit_cable_full_whip(job,trials,engine):
    ...
    accelerator=CudaCableFit(engine,data,chosen,block_steps=3)
    ...
    protocol['cable_execution_backend']='recomputed_cuda_vjp_blocks_v1_float64'
    protocol['cable_graph_block_steps']=3;save(job/'protocol.json',protocol)
