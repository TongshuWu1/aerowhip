"""Fit a bounded cable-only residual to preliminary motion, with physics frozen.
...
def prepare_residual_job(root, *, updates=None):
    ...
    atomic_json(job/'provenance.json', dict(model_sha256=canonical_json_hash(model),
        ...
        whip_data_used=False, drone_residual_trained=False, physics_frozen=True,
...
def fit(job):
    ...
    progress('Preparing preliminary cable motion; physical calibration frozen')
    ...
    for update in range(1, settings['updates']+1):
        ...
        if update % settings['evaluate_every'] == 0 or update == settings['updates']:
            ...
            # The helper reports an empty validation mean here, which we do not use.
            metrics, _, _ = evaluate_together(selection_takes, model, cable, parameters)
            current = metrics['training']['objective']
    ...
    atomic_json(job/'review.json', dict(accepted=accepted and best_update>0, validation_checks=checks,
        ...
        selection='Training-only 2-second rollout objective, including zero residual',
        physics_frozen=True, active_model_changed=False, drone_residual_trained=False,
...
def apply_candidate(root, job):
    ...
    atomic_json(folder/'manifest.json', dict(provenance='Validated preliminary cable residual; frozen physics', source=str(job)))
