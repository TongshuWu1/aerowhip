...
def fit_cable(job):
    ...
    output=job/'cable';output.mkdir(exist_ok=False)
    ...
    for fold,excluded in enumerate([*settings['folds'],[]]):
        ...
        check=gradient_check(combine([subset(t,[0]) for t in train_short]),model,cable,params,network)
...
def main():
    ...
    try:
        (fit_cable if args.stage=='cable' else fit_drone)(args.job)
