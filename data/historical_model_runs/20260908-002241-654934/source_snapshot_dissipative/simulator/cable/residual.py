...
class MotionResidual(nn.Module):
    def __init__(self, node_count, *, hidden=48, acceleration_limit=2.0,
        ...
        if mode not in ('acceleration','dissipative'):raise ValueError('Unknown cable residual mode')
        self.mode=mode;self.damping_limit_s_inv=float(damping_limit_s_inv)
        if mode=='dissipative' and (learn_drag or not math.isfinite(self.damping_limit_s_inv) or self.damping_limit_s_inv<=0):
    ...
    def forward(self, positions, velocities):
        ...
        raw=self.net(features).reshape(-1,self.node_count-1,3)
