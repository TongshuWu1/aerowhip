...
def write_rehearsal_fullstate(arrays, summary, directory, source_metadata, *, gentle_recovery=False,
                             initial_world_attachment_offset=None):
...
def convert_attachment_reference(directory, metadata, offset):
...
"""
    ...
    metadata['reference_mapping']=dict(initial_world_attachment_offset_m=offset.tolist(),
        ...
        velocity_acceleration='unchanged; constant initial reference-frame offset',
        original_attachment_csv=backup.name,
        initial_pose_assumption='Offline rehearsal assumes level, zero-yaw hover; actual initial pose adapter must rotate the body offset')
