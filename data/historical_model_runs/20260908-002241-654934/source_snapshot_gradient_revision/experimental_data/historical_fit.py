...
def fit_cable(job, output_name='cable'):
    ...
    for fold,excluded in enumerate([*settings['folds'],[]]):
        ...
        network=MotionResidual(cable.node_count,hidden=settings['cable_hidden'],acceleration_limit=settings['cable_acceleration_limit']).to(device='cuda',dtype=torch.float64)
        model.motion_residual=network
        probe=combine([subset(t,[0]) for t in train_short])
        checks=[gradient_check(probe,model,cable,params,network,epsilon=e) for e in (1e-4,3e-5)]
        check=dict(passed=all(c['passed'] for c in checks),steps=checks,
            reason='Two local finite-difference scales; initial 1e-3 perturbation failed but converged as epsilon decreased')
        ...
        network.load_state_dict(best);save_weights(folder/'cable_residual.pt',network)
    ...
    atomic_json(job/'cable_result.json',dict(directory=output_name))
    ...
    for name,a in data.items():
        ...
        for s in starts:
            ...
            rows.append(dict(name=name,start=int(s),position=a['position'][s:s+count+1],velocity=velocity,
                commands=a['commands'][s-20:s+count],time=a['time'][s:s+count+1]))
    return rows
...
def fit_drone(job):
    ...
    output=job/'drone';output.mkdir(exist_ok=False)
    ...
    for fold,excluded in enumerate([*settings['folds'],[]]):
        ...
        checkpoint=dict(schema='effective_fullstate_drone_residual_v1',specification=net.specification(),
            ...
            reference_point='OptiTrack cf_7 origin',flight_ready=False)
...
def main():
    ...
    parser.add_argument('--job',type=Path);parser.add_argument('--stage',choices=('cable','drone'))
    ...
    try:
        if args.stage=='cable':fit_cable(args.job,args.output_name)
        else:fit_drone(args.job)
