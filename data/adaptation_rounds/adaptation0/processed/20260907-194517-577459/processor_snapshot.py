...
def read_optitrack(path):
    ...
    def columns(name, kind, axes):
        ...
        return result
    ids = [0,1]+columns('cf_7','Position','XYZ')+columns('cf_7','Rotation','XYZW')
    ...
    return dict(frame=a[:,0], time=a[:,1], drone=a[:,2:5], quaternion=a[:,5:9],
                cable=a[:,9:].reshape(-1,10,3), metadata=meta)
...
def read_controller(path):
    ...
    required = ['time_s','x','y','z','cmd_age','cmd_valid',*COMMAND_COLUMNS]
    if not set(required).issubset(c.dtype.names or []):
        raise ValueError('Controller CSV must contain measured XYZ and full-state command fields')
...
def import_round(root, source, number, *, notes='', parent=None, reference=None, provenance=None):
    ...
    for trial, tracking, controller in pairs:
        ...
        records.append(dict(trial_id=trial,sources=sources))
    ref_info=None
    if reference:
    ...
    write_json(destination/'round.json',dict(schema='aerial_whip_recording_round_v1',round=name,
        ...
        identity=dict(drone='cf_7',cable=[f'cable1:c{i}' for i in range(1,11)],unlabeled='ignored'),
...
def align(m,c,override=None):
    ...
    if override is None:
        ...
        result=minimize_scalar(cost,bounds=(max(lo,best-.02),min(hi,best+.02)),method='bounded')
...
def process_round(directory, overrides=None):
    ...
    for trial in meta['trials']:
        ...
        try:
            ...
            valid_c=(c['cmd_valid']==1)&np.isfinite(vals).all(axis=1)&np.isfinite(c['cmd_age'])&(c['cmd_age']>=0)
            cmd_age=t-ct[idx]+c['cmd_age'][idx]
            valid=valid_c[idx]&(cmd_age<=.1)&(t<=ct[-1])
            ...
            qvalid=np.isfinite(m['quaternion']).all(axis=1)
            ...
            np.savez_compressed(folder/'dataset.npz',optitrack_time_s=m['time'],controller_time_s=t,
                ...
                cable_names=np.array(meta['identity']['cable']),reference_columns=np.array(COMMAND_COLUMNS))
            ...
            report=dict(trial_id=name,status='processed — review required',alignment=sync,
                ...
                conventions=dict(position='source global meters; no frame transform',quaternion='source OptiTrack XYZW; mapping to body frame unverified',
                    ...
                    reference='kinematic full-state commands, not forces; causal hold with 100 ms age limit'),
                output_files={p.name:sha256_file(p) for p in folder.iterdir() if p.is_file()})
            write_json(folder/'quality.json',report)
            plot_review(folder,m,t,c,segments)
            reports.append(report)
        except Exception as error:
            reports.append(dict(trial_id=name,status='failed',error=str(error),training_ready=False))
    write_json(output/'processing.json',dict(schema='aerial_whip_processed_round_v1',created_utc=utc_now(),
...
def plot_review(folder,m,t,c,segments):
    import matplotlib
    matplotlib.use('Agg')
    ...
    for j,n in enumerate(('x','y','z')):
        ...
        good=(c['cmd_valid']==1)&(c['cmd_age']<=.1)
        ax.step(c['time_s'][good],c['cmd_'+n][good],where='post',label='Desired',alpha=.7)
    ...
    fig.suptitle(folder.name+' — alignment and data quality');fig.tight_layout()
