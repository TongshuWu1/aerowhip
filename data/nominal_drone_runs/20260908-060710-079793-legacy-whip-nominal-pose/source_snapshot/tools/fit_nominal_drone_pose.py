...
def prepare():
    ...
    protocol=dict(schema='nominal_drone_pose_bootstrap_fit_v1',scope='Legacy whip-only nominal drone; no residual or cable fit',
        ...
        attitude_search='Fixed eleven-point geometric grid from 0.02 to 0.30s, then bounded refinement in the best grid cell',
...
def run(job):
    ...
    for label,training in [('final_all_three',trials)]+[('leave_out_'+t.name,[x for x in trials if x.name!=t.name]) for t in trials]:
        ...
        param_values=dict(zip(GAIN_NAMES,map(float,gains)),attitude_time_constant_s=tau,delay_s=delay,gravity_m_s2=9.80665)
        model=dict(schema='nominal_loaded_drone_pose_candidate_v1',parameters=param_values,
        ...
        for trial in trials:
            ...
            if not rejection:
                try:result,availability=diagnostic_pose(trial,gains,tau,delay)
