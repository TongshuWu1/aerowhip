...
class PreparedTrial:
    ...
    @torch.no_grad()
    def pose(self,gains,tau,delay,*,device='cuda',step=.005,maneuver_only=False):
        if device=='cuda':
        ...
        return predict_pose(self.state(gains,device,tau,delay),stream,times,parameters(gains,tau,delay),offset_tracking_m=self.offset,maximum_step_s=step)
...
def diagnostic_pose(trial,gains,tau,delay):
    ...
    """
    try:
        return trial.pose(gains,tau,delay),dict(status='FULL_DIAGNOSTIC_PREDICTED',maneuver_only=False)
    except ValueError as exc:
        if str(exc)!='Attitude error is too close to 180 degrees for this local response model':raise
        result=trial.pose(gains,tau,delay,maneuver_only=True)
