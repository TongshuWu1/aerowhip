...
from experimental_data.nominal_pose_fit import (PreparedTrial,read_review,fit_translation,fit_attitude,
    score_pose,diagnostic_pose,parameters,PRIOR,LOWER,UPPER,SCALES,PRIOR_WEIGHT,POSITION_SCALE_M,GAIN_NAMES,DELAYS,STARTS)
...
def prepare():
    ...
    protocol=dict(schema='nominal_drone_pose_bootstrap_fit_v1',scope='Legacy whip-only nominal drone; no residual or cable fit',
        ...
        domain_failure_reporting='Reject invalid attitude parameter candidates; abort on selected maneuver-domain failure; mark entire post-hold diagnostic unavailable when only extended replay fails. Never impute a recovery.',
...
def run(job):
    ...
    for label,training in [('final_all_three',trials)]+[('leave_out_'+t.name,[x for x in trials if x.name!=t.name]) for t in trials]:
        ...
        for trial in trials:
            result,availability=diagnostic_pose(trial,gains,tau,delay)
            ...
            fine=trial.pose(gains,tau,delay,step=.0025,maneuver_only=availability['maneuver_only'])
    ...
    for i,trial in enumerate(trials):
        ...
        for value,label,style in [(a,'All-three fit','-'),(b,'Left-out take prediction','--')]:
            ...
            error[valid]=Rotation.from_matrix(value['rotation_tracking_to_world'][valid].transpose(0,2,1)@trial.truth['rotation_tracking_to_world'][valid]).magnitude()
