...
from experimental_data.nominal_pose_fit import (fitting_masks,integration_steps,linear_prediction,
    translation_objective,fit_translation,fit_attitude,diagnostic_pose,score_pose,translation_only_diagnostic,parameters,PRIOR,GAIN_NAMES)
...
def test_post_hold_failure_is_unavailable_and_maneuver_failure_is_never_hidden():
    t,truth=truth_fixture()
    truth['rotation_tracking_to_world']=np.broadcast_to(np.eye(3),(len(t),3,3)).copy()
    truth['position_attachment_m']=np.zeros((len(t),3))
    domain='Attitude error is too close to 180 degrees for this local response model'
    def pose(*args,maneuver_only=False,**kwargs):
        if not maneuver_only:raise ValueError(domain)
        return dict(rotation_tracking_to_world=torch.eye(3,dtype=torch.float64).expand(1,90,3,3),
            position_origin_m=torch.zeros(1,90,3),position_attachment_m=torch.zeros(1,90,3))
    trial=SimpleNamespace(time=t,pose=pose,truth=truth,masks=fitting_masks(t,truth))
    result,status=diagnostic_pose(trial,PRIOR,.08,.02)
    arrays,scores=score_pose(trial,result)
    assert status['status']=='POST_HOLD_DOMAIN_FAILURE'
    assert np.isnan(arrays['position_origin_m'][90:]).all()
    assert scores['csv_maneuver']['origin_rmse_m']==0
    hold=scores['post_maneuver_fullstate_hold']
    assert hold['origin_measured_samples']==31 and hold['origin_samples']==0
    assert hold['origin_rmse_m'] is None and hold['orientation_rmse_deg'] is None
    def always_fail(*args,**kwargs):raise ValueError(domain)
    trial.pose=always_fail
    with pytest.raises(ValueError,match='180 degrees'):diagnostic_pose(trial,PRIOR,.08,.02)
    trial.name='synthetic'
    rejected=fit_attitude([trial],PRIOR,.02,device='cpu',progress=lambda *a,**k:None)
    assert not rejected['valid'] and rejected['tau_s'] is None
    assert len(rejected['trace'])==11 and not any(row['valid'] for row in rejected['trace'])
    trial.linear=lambda *args,**kwargs:(np.zeros((len(t),3)),np.zeros((len(t),3)),None)
    arrays,scores,status,convergence=translation_only_diagnostic(trial,PRIOR,.02,'No valid attitude')
    assert status['status']=='POSE_CANDIDATE_REJECTED'
    assert scores['csv_maneuver']['origin_rmse_m']==0
    assert scores['csv_maneuver']['orientation_rmse_deg'] is None
    assert scores['csv_maneuver']['attachment_samples']==0
    assert np.isnan(arrays['rotation_tracking_to_world']).all()
