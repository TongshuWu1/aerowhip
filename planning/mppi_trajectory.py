...
def candidate_better(value,hit,best,best_hit,contact_priority):
    """Explicit flight-candidate ordering; the soft objective itself is unchanged."""
    return (bool(hit),value)>(bool(best_hit),best) if contact_priority else value>best


def candidate_index(score,success,failed,contact_priority):
    valid=torch.isfinite(score)&~failed
    if not bool(valid.any()):raise ValueError('No feasible candidate for selection')
    hits=valid&success
    if contact_priority and bool(hits.any()):valid=hits
    return int(score.masked_fill(~valid,-torch.inf).argmax())
...
class ObjectiveCapture(RolloutCapture):
    ...
    def score(self,env,result,actions,weights):
        ...
        if env.settings.get('reward',{}).get('early_hit',0.):
            from learning.pva_success import hit_time_bonus
            terms['early_hit']=hit_time_bonus(env.settings['reward'],result['success'],result['failed'],result['duration_s'])
        if env.settings.get('reward',{}).get('impact',0.):
            from learning.pva_success import impact_bonus
            terms['impact']=impact_bonus(env.settings['reward'],result['success'],result['failed'],env.hit_tip_velocity,env.direction)
...
def optimize(job,model,cfg):
    ...
    if timed:
        with np.load(job/'proposal_baselines.npz') as data:
            baselines=torch.as_tensor(data['normalized_jerk'],device=device,dtype=torch.float64)
        if baselines.shape!=(2,steps,3) or not bool(torch.isfinite(baselines).all()) or bool((baselines.abs()>1).any()):
            raise ValueError('Two complete, finite, bounded baseline command sequences required')
    ...
    env=PVAEnvironment(model,cfg,root=job,batch_size=samples+groups+(3 if timed else 2),device=device)
    ...
    if timed:
        if not bool(torch.isfinite(score[-2:]).all()):raise ValueError('Known baseline became infeasible')
        baseline_scores=score[-2:].cpu().tolist()
        baseline_hits=result['success'][-2:].cpu().tolist()
        chosen=[g*per+int(score[g*per:(g+1)*per].argmax()) for g in range(groups)]
        means=controls[chosen].clone()
        # Two native-centered families, plus their screened timing/strength basins.
        means[:2].zero_()
        atomic_json(job/'baseline_evaluation.json',dict(names=['editable_command_baseline_1','editable_command_baseline_2'],
            scores=baseline_scores,objective_components={k:v[-2:].cpu().tolist() for k,v in terms.items()},
            distances_m=result['minimum_tip_distance_m'][-2:].cpu().tolist(),
            success=result['success'][-2:].cpu().tolist(),failed=result['failed'][-2:].cpu().tolist()))
    else:
    ...
    history=[];anchor=-math.inf;anchor_hit=False;improved_at=0;iteration=0
    ...
    while not s['iterations'] or iteration<s['iterations']:
        ...
        if timed and any(candidate_better(v,h,best+1e-7,best_hit,contact_priority)
                         for v,h in zip(baseline_scores,baseline_hits)):
            raise ValueError('Known baseline was lost under the configured candidate ordering')
        if not math.isfinite(anchor) or (contact_priority and best_hit and not anchor_hit) or best>anchor+max(.1,abs(anchor)*.005):
            anchor=best;anchor_hit=best_hit;improved_at=iteration
        ...
        if timed:row.update(baseline_scores=baseline_scores,improvement_over_best_baseline=best-max(baseline_scores))
        if cfg.get('visualization',{}).get('live_mppi',True):
            labels={samples+g:f'Proposal mean {g}' for g in range(groups)} if timed else None
