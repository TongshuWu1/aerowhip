...
from .spline import fit_seed, sample, polynomial_feasible
from .cem import optimize
from .cem_objective import resolve_reward,task_with_settings
...
MPPI_DEFAULTS = dict(DEFAULTS, optimizer='mppi', temperature=20., display_name='M1 MPPI spline',
    population=128, position_std_m=.005, duration_std_s=.02,
    model_path='data/model_candidates/20260908-adp0-M1/model.json')


def validate_settings(settings):
    if settings.get('optimizer', 'cem') not in ('cem', 'mppi'):
        raise ValueError('Unknown spline optimizer.')
    if settings.get('optimizer') == 'mppi' and (not np.isfinite(settings.get('temperature', 0)) or settings['temperature'] <= 0):
        raise ValueError('MPPI temperature must be positive and finite.')
    resolve_reward(settings.get('reward'))
    if 'launch_setup' in settings:resolve_launch_setup(settings['launch_setup'])
...
def prepare_job(root, seed_directory, output, settings, origin=None, target=None):
    ...
    defaults = MPPI_DEFAULTS if settings.get('optimizer') == 'mppi' else DEFAULTS
    ...
    if meta.get('schema') not in ('research_fullstate_30hz_v1', 'cem_fullstate_30hz_v1', 'mppi_fullstate_30hz_v1'):
    ...
    model_source = seed_directory/'model.json'
    if settings.get('model_path'):
        model_source = Path(settings['model_path'])
        if not model_source.is_absolute():
            model_source = root/model_source
        model_source = model_source.resolve()
        model = read_json(model_source)
    settings['model_path'] = str(model_source)
    for name in ('motion_residual','fullstate_execution'):
        path = Path(model[name]['checkpoint'])
        if not path.is_absolute():
            model[name]['checkpoint'] = str((model_source.parent/path).resolve())
    from simulator.research_config import validate_research_contract
    task=task_with_settings(task,settings)
    settings['reward']=resolve_reward(settings.get('reward'))
    settings['success']=task['success'].copy()
    settings['desired_strike_direction_world']=task['desired_strike_direction_world'].copy()
    validate_research_contract(model, task, config)
    saved_launch=read_json(root/f'config/{settings.get("optimizer","cem")}.json',{}).get('launch_setup')
    launch=resolve_launch_setup(settings.get('launch_setup',saved_launch))
    if origin is not None:launch['initial_tracking_origin_m']=origin
    if target is not None:launch['target_position_m']=target
    settings['launch_setup']=resolve_launch_setup(launch)
    origin=np.asarray(settings['launch_setup']['initial_tracking_origin_m'])
    target=np.asarray(settings['launch_setup']['target_position_m'])
    ...
    atomic_json(output/'model_provenance.json', dict(source=str(model_source),
        source_sha256=sha256_file(model_source), snapshot_sha256=sha256_file(output/'model.json'),
        usage='Model for every new optimization and complete prediction; seed trajectory is initialization only'))
    for name, value in [('task',task),('ppo',config),('cem',settings)]:
        atomic_json(output/f'{name}.json', value)
    if settings.get('optimizer') == 'mppi':
        atomic_json(output/'mppi.json', settings)
...
def run_job(output):
    ...
    if read_json(output/'run.json').get('status') != 'PREPARED':
        raise ValueError('This run has already started. Create a new run to preserve its saved results.')
    # Exclusive ownership also prevents two launchers racing on PREPARED.
    with (output/'worker.lock').open('x',encoding='utf-8') as lock:
        lock.write(str(os.getpid()))
    settings = read_json(output/'cem.json'); validate_settings(settings)
    algorithm = settings.get('optimizer', 'cem').upper()
    ...
    atomic_json(output/'run.json', dict(status='RUNNING', display_name=settings['display_name'],pid=os.getpid()))
    try:
        ...
        with np.load(output/'seed.npz') as data:
            ...
            if algorithm=='MPPI':
                from .spline import initialize_at_origin
                coefficients = initialize_at_origin(data['commands'], duration, settings['control_points'], evaluator.origin)
                atomic_json(output/'seed_initialization.json',dict(method='relative_seed_at_new_origin_before_optimization',
                    seed_origin_m=data['commands'][0,:3].tolist(),initial_tracking_origin_m=evaluator.origin.tolist(),
                    saved_outputs_transformed=False,requires_fresh_model_evaluation=True))
            else:
                coefficients = fit_seed(data['commands'], duration, settings['control_points'])
        ...
        def progress(row, best, bank):
            ...
            atomic_json(output/'progress.json', dict(label=f'{algorithm} iteration {row["iteration"]}/{settings["iterations"]} · best {row["best_score"]:.2f} · population hits {row["success_fraction"]:.0%}',step=row['iteration'],total=settings['iterations'],**row))
            print(json.dumps(row), flush=True)
        if algorithm == 'MPPI':
            from .mppi import optimize as optimize_mppi
            _, _, bank = optimize_mppi(mean,std,evaluator,population=settings['population'],iterations=settings['iterations'],
                temperature=settings['temperature'],seed=settings['random_seed'],progress=progress,cancelled=cancelled)
        else:
        ...
        for score, vector in bank:
            ...
            hit = evaluator.rollout([whip],[duration],record=True)
            if hit['failed'][0]:
            ...
            height_bounds=(settings['minimum_height_m'],settings['maximum_height_m'])
            segments=[(recovery['turn_coefficients_normalized'],recovery['turn_end_s'])]
            if recovery['approach_coefficients_normalized'] is not None:
                segments.append((recovery['approach_coefficients_normalized'],recovery['return_s']))
            if not all(polynomial_feasible(np.asarray(poly),length,model['fullstate_execution']['feasibility'],height_bounds) for poly,length in segments):
                rejected.append('Continuous recovery exceeds envelope/height between packets');continue
            ...
            prefix=(len(whip)-1)*5+1
            prefix_error=max(float(np.max(np.abs(prediction['origin'][:prefix]-hit['origin'][:prefix]))),
                             float(np.max(np.abs(prediction['cable'][:prefix]-hit['cable'][:prefix]))))
            if prefix_error > 1e-7:
                rejected.append(f'Whip/recovery prediction prefix mismatch: {prefix_error:g} m');continue
        ...
        meta=dict(schema=f'{algorithm.lower()}_fullstate_30hz_v1',planner=f'{algorithm} quintic position spline',display_name=settings['display_name'],
            ...
            whip_export_prefix_max_difference_m=prefix_error,
            prediction_valid_through_s=float(times[-1]),reference_feasible=True,
            continuous_command_envelope_checked=True,prediction_rate_hz=150,
            ...
            model_provenance=read_json(output/'model_provenance.json',{}),optimizer=algorithm.lower(),
...
def export_package(directory,destination):
    ...
    if meta.get('schema') not in ('cem_fullstate_30hz_v1','mppi_fullstate_30hz_v1','mppi_force_fullstate_30hz_v1'):
        raise ValueError('Select a completed spline planner result.')
    ...
    with zipfile.ZipFile(destination,'w',compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        archive.writestr('README.txt',meta.get('planner','Spline planner')+' / native 30 Hz FullState offline result\n'
