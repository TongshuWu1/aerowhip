...
SUPPORTED_FIXED_NODE_COUNTS = (11, 12, 21, 31)
...
def _block_threads(node_count: int, pinned_start_nodes: int = 1) -> int:
...
def _specialized_mechanics_source(
    node_count: int,
    *,
    pinned_start_nodes: int = 1,
    ...
    iteration_count = (
        6 * (node_count - pinned_start_nodes)
        if iterations is None
        else int(iterations)
    )
    threads = _block_threads(node_count, pinned_start_nodes)
    base_source = (
        _SOURCE
        if iteration_count == 60
        else _experimental_pcg_source(iteration_count)
    )
    start = base_source.index(
        'extern "C" __device__ __forceinline__ void skew3'
    )
    source = base_source[start:]
    damping_name = (
        f"fixed_damping_{node_count}node_{iteration_count}pcg"
        if pinned_start_nodes == 1
        else f"fixed_damping_{node_count}node_{pinned_start_nodes}pinned_"
        f"{iteration_count}pcg"
    )
    projection_name = (
        f"fixed_projection_{node_count}node_4plus1"
        if pinned_start_nodes == 1
        else f"fixed_projection_{node_count}node_{pinned_start_nodes}pinned_4plus1"
    )
    source = source.replace(
        f"fixed_damping_11node_{iteration_count}pcg",
        damping_name,
    )
    source = source.replace(
    ...
    source = source.replace(
        "constexpr int ITERATIONS = 60;",
        f"constexpr int ITERATIONS = {iteration_count};",
    )
    source = source.replace(
        "__shared__ double direction_shared[32];",
    ...
    source = source.replace(
        "column_node < 10", f"column_node < N - {pinned_start_nodes}"
    )
    source = source.replace(
...
#pragma unroll
    ...
    __syncthreads();"""
        new_assembly = r"""    constexpr int P = 2;
    const long long q_base = (long long)system_index * N * 3;
    const long long boundary_base = (long long)system_index * P * 3;
    if (lane == 0) {
#pragma unroll
        for (int joint = 0; joint < N - 2; ++joint) {
            float previous_edge[3], following_edge[3];
...
#pragma unroll
                for (int column = 0; column < 3; ++column) {
                    const float previous_value = -previous_block[row * 3 + column];
                    const float middle_value = previous_block[row * 3 + column]
                        - following_block[row * 3 + column];
                    const float following_value = following_block[row * 3 + column];
                    if (joint < P) {
                        prescribed[output_row] += previous_value
                            * boundary_velocities[boundary_base + joint * 3 + column];
                    } else {
                        jacobian[output_row * D + (joint - P) * 3 + column]
                            += previous_value;
                    }
                    if (joint + 1 < P) {
                        prescribed[output_row] += middle_value
                            * boundary_velocities[boundary_base + (joint + 1) * 3 + column];
                    } else {
                        jacobian[output_row * D + (joint + 1 - P) * 3 + column]
                            += middle_value;
                    }
                    jacobian[output_row * D + (joint + 2 - P) * 3 + column]
                        += following_value;
                }
            }
        }
    ...
    __syncthreads();"""
        if old_assembly not in source:
            raise RuntimeError("Could not generalize the fixed damping assembly.")
        source = source.replace(old_assembly, new_assembly)
        source = source.replace(
            "const int node = lane / 3 + 1;",
            "const int node = lane / 3 + P;",
        )
        source = source.replace(
            "undamped_velocities[q_base + (lane + 3)]",
            "undamped_velocities[q_base + (lane + 3 * P)]",
        )
        old_output = r"""    if (lane < 3) {
        output_velocities[q_base + lane] = boundary_velocities[
            (long long)system_index * 3 + lane
        ];
    }
    if (lane < D) output_velocities[q_base + lane + 3] = (float)solution;"""
        new_output = r"""    if (lane < 3 * P) {
        output_velocities[q_base + lane] = boundary_velocities[boundary_base + lane];
    }
    if (lane < D) output_velocities[q_base + lane + 3 * P] = (float)solution;"""
        if old_output not in source:
            raise RuntimeError("Could not generalize the fixed damping output.")
        source = source.replace(old_output, new_output)

        source = source.replace(
            "const long long boundary_base = (long long)sample * 3;",
            "constexpr int P = 2;\n"
            "    const long long boundary_base = (long long)sample * P * 3;",
        )
        source = source.replace(
            r"""#pragma unroll
    for (int axis = 0; axis < 3; ++axis) {
...
#pragma unroll
        for (int axis = 0; axis < 3; ++axis) {
            value[node * 3 + axis] = boundary_positions[
                boundary_base + node * 3 + axis
            ];
        }
    }
#pragma unroll
    for (int node = 0; node < N; ++node) {
        inverse_mass[node] = node < P ? 0.0f : 1.0f / masses[node];
    }""",
        )
        position_reset = r"""#pragma unroll
        for (int axis = 0; axis < 3; ++axis) {
            value[axis] = boundary_positions[boundary_base + axis];
        }"""
        position_reset_new = r"""#pragma unroll
        for (int node = 0; node < P; ++node) {
#pragma unroll
            for (int axis = 0; axis < 3; ++axis) {
                value[node * 3 + axis] = boundary_positions[
        ...
        source = source.replace(position_reset, position_reset_new)
        velocity_reset = r"""#pragma unroll
    for (int axis = 0; axis < 3; ++axis) {
        velocity[axis] = boundary_velocities[boundary_base + axis];
    }"""
        velocity_reset_new = r"""#pragma unroll
    for (int node = 0; node < P; ++node) {
#pragma unroll
        for (int axis = 0; axis < 3; ++axis) {
            velocity[node * 3 + axis] = boundary_velocities[
                boundary_base + node * 3 + axis
            ];
        }
    }"""
...
class _FixedPcgKernel:
    def __init__(
        ...
        if pinned_start_nodes not in (1, 2):
            raise ValueError("Fixed mechanics supports one or two pinned start nodes.")
        self.pinned_start_nodes = pinned_start_nodes
        self.backend = validate_damping_backend(backend)
        if (
            self.backend == "block_banded_direct_experimental"
            and (node_count != 11 or pinned_start_nodes != 1)
        ):
            raise ValueError(
                "The experimental block-banded direct backend is 11-node pivot only."
            )
        self.degrees_of_freedom = 3 * (node_count - pinned_start_nodes)
        self.iterations = (
            6 * (node_count - pinned_start_nodes)
        ...
        preserve_legacy_source = (
            node_count == 11
            and pinned_start_nodes == 1
            and self.backend == REFERENCE_DAMPING_BACKEND
        )
        ...
        elif node_count == 11 and pinned_start_nodes == 1:
            source = _experimental_pcg_source(self.iterations)
        else:
            source = _specialized_mechanics_source(
                node_count,
                pinned_start_nodes=pinned_start_nodes,
        ...
        ptx = self._compile_ptx(
            ...
            source=source,
            source_name=(
                f"fixed_mechanics_{node_count}node_{pinned_start_nodes}pinned_"
                f"{self.backend}.cu"
            ),
        )
        ...
        if node_count == 11 and pinned_start_nodes == 1:
        ...
        elif pinned_start_nodes == 2:
            damping_name = (
                f"fixed_damping_{node_count}node_2pinned_{self.iterations}pcg".encode(
                    "ascii"
                )
            )
        else:
        ...
        projection_name = (
            f"fixed_projection_{node_count}node_4plus1"
            if pinned_start_nodes == 1
            else f"fixed_projection_{node_count}node_2pinned_4plus1"
        ).encode("ascii")
    ...
    def launch_damping(
        ...
        boundary_shape = (batch, self.pinned_start_nodes, 3)
        if boundary_velocities.shape != boundary_shape:
            raise ValueError(
                f"Fixed damping requires boundary velocities shaped {boundary_shape}."
            )
        tensors = (
            positions,
            undamped_velocities,
    ...
    def launch_projection(
        ...
        boundary_shape = (batch, self.pinned_start_nodes, 3)
        if (
            boundary_positions.shape != boundary_shape
            or boundary_velocities.shape != boundary_shape
        ):
            raise ValueError(
                f"Fixed projection requires boundary tensors shaped {boundary_shape}."
            )
        tensors = (
            predicted_positions,
            previous_positions,
...
_kernels: dict[tuple[int, DampingBackend, int], _FixedPcgKernel] = {}
...
def _kernel_for(
    ...
    if pinned_start_nodes not in (1, 2):
        raise ValueError("Fixed mechanics supports one or two pinned start nodes.")
    key = (node_count, backend, pinned_start_nodes)
    kernel = _kernels.get(key)
    if kernel is None:
        with _lock:
            kernel = _kernels.get(key)
            if kernel is None:
                kernel = _FixedPcgKernel(node_count, backend, pinned_start_nodes)
                _kernels[key] = kernel
    return kernel
...
def fixed_damping_supported_nodes(
    ...
    *,
    ...
    """Apply the specialized homogeneous-cable damping for supported node counts."""

    return _kernel_for(
        int(positions.shape[1]), backend, pinned_start_nodes
    ).launch_damping(
        positions,
        undamped_velocities,
        boundary_velocities,
...
def fixed_projection_supported_nodes(
    ...
    *,
    ...
    """Apply four position plus one velocity projection for supported node counts."""

    return _kernel_for(
        int(predicted_positions.shape[1]),
        REFERENCE_DAMPING_BACKEND,
        pinned_start_nodes,
    ).launch_projection(
        predicted_positions,
