...
# distal end remains mechanically free.  No material-frame/torsion state is
# introduced; this is strictly a position+tangent boundary condition.
CLAMPED_START_FREE_END: PinnedEndpointMask = (2, False)
...
def _validate_pinned_endpoints(
    ...
    *,
    ...
    if not isinstance(pinned_endpoints, tuple) or len(pinned_endpoints) != 2:
        raise ValueError("pinned_endpoints must contain start/end pin counts.")
    if any(type(value) not in (bool, int) for value in pinned_endpoints):
        raise ValueError("Pinned-node counts must be booleans or integers.")
    ...
    if supported_only and normalized not in supported:
        raise ValueError(
            ...
            "a free end, or a two-node clamped start with a free end."
        )
    return normalized
...
def _pinned_values(
    ...
    start_count, end_count = (int(value) for value in pinned_endpoints)
    parts = []
    if start_count:
        parts.append(values[:, :start_count])
    if end_count:
        parts.append(values[:, -end_count:])
    if not parts:
        return values[:, :0]
...
def _combine_boundary_and_free_values(
    ...
    parts = []
    if start_count:
        parts.append(boundary_values[:, :start_count])
    parts.append(free_values)
    if end_count:
        parts.append(boundary_values[:, start_count : start_count + end_count])
    return torch.cat(parts, dim=1)
...
def _implicit_bending_damping_velocity(
    ...
    *,
    ...
    fixed_runtime_damping = (
        ...
        and conjugate_gradient_iterations
        == 6 * (positions.shape[1] - int(pinned_endpoints[0]))
        and not positions.requires_grad
        and not undamped_velocity.requires_grad
        and not boundary_velocity.requires_grad
        and not damping.requires_grad
    ...
    if fixed_runtime_damping:
        ...
        return fixed_damping_supported_nodes(
            ...
            pinned_start_nodes=int(pinned_endpoints[0]),
        )
...
def momentum_project_lengths(
    ...
    *,
    ...
    start_count, end_count = (int(value) for value in effective_pins)
    if start_count:
        inverse_mass = torch.cat(
            (torch.zeros_like(inverse_mass[:start_count]), inverse_mass[start_count:])
        )
    if end_count:
        inverse_mass = torch.cat(
            (inverse_mass[:-end_count], torch.zeros_like(inverse_mass[-end_count:]))
        )
...
class DderModel:
    ...
    def __init__(self, parameters: DderParameters) -> None:
        ...
        self._maximum_bending_eigenvalue_per_ei_clamped_start = (
            maximum_free_eigenvalue(
                interior_unit_stiffness,
    ...
    def _maximum_bending_eigenvalue(
        self,
        pinned_endpoints: PinnedEndpointMask,
    ) -> float:
        normalized = _validate_pinned_endpoints(
            pinned_endpoints,
            supported_only=True,
        )
        if normalized == (1, 1):
    ...
    def maximum_stable_bending_stiffness(
        ...
        *,
        ...
        maximum_eigenvalue = self._maximum_bending_eigenvalue(pinned_endpoints)
    ...
    def _runtime_internal_force(
        ...
        *,
        ...
        with torch.enable_grad():
            q = (
                positions
                if create_graph and positions.requires_grad
            ...
            if endpoint_orientations is not None:
                terminal_curvature = _terminal_curvature_binormals_impl(
                    ...
                    endpoint_orientations
                    if create_graph
                    else endpoint_orientations.detach(),
                    validate=False,
                )
            ...
            return -torch.autograd.grad(
                energy.sum(), q, create_graph=create_graph
            )[0]

    @staticmethod
    def _runtime_internal_force_functional(
        positions: torch.Tensor,
        constants: DderRuntimeConstants,
        endpoint_orientations: torch.Tensor | None = None,
        endpoint_twist_reference_rad: torch.Tensor | None = None,
    ) -> torch.Tensor:
        """Same energy gradient expressed as a composable ``torch.func`` transform.

        This exists solely for local solver linearization.  It evaluates the
        identical bending/twist energy as :meth:`_runtime_internal_force`; the
        different implementation avoids ``requires_grad_`` inside an outer
        JVP transform.
        """
        ...
        def total_energy(q: torch.Tensor) -> torch.Tensor:
            ...
            if endpoint_orientations is not None:
                terminal_curvature = _terminal_curvature_binormals_impl(
                    q, endpoint_orientations, validate=False
                )
                terminal_dual = torch.stack(
                    (constants.rest_lengths_m[0], constants.rest_lengths_m[-1])
                )
                ...
                twist = _angle_nearest_reference(
                    twist, endpoint_twist_reference_rad
                )
                energy = energy + 0.5 * constants.torsional_stiffness_n_m2 * (
                    twist.square() / torch.sum(constants.rest_lengths_m)
                )
        ...
        return -torch.func.grad(total_energy)(positions)
    ...
    def step(
        ...
        *,
        ...
        one_attached = pinned_endpoints in ((1, 0), (2, 0))
        if one_attached and (
        ...
        *,
        ...
        for substep in range(self.parameters.substeps):
            ...
            force = (
                ...
                if functional_force_autograd
                else self._runtime_internal_force(
                    q,
                    constants,
                    orientations,
                    continuous_twist,
            ...
            fixed_runtime_projection = (
                ...
                and not functional_force_autograd
                and predicted_q.shape[1] in (11, 12, 21, 31)
                and pinned_endpoints
                in (START_PINNED_FREE_END, CLAMPED_START_FREE_END)
                and self.parameters.constraint_iterations == 4
                and not predicted_q.requires_grad
