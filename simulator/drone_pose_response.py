...
def nominal_acceleration(position,velocity,compensation,command,parameters):
    kp,kd,ff,_=parameters.tensors(position)
    return kp*(command[:,:3]-position)+kd*(command[:,3:6]-velocity)+ff*command[:,6:9]+compensation


def derivatives(state,command,parameters,residual=None):
    kp,kd,ff,tau=parameters.tensors(state.position)
    acceleration=nominal_acceleration(state.position,state.velocity,state.compensation,command,parameters)
    ...
    # The NN describes additional realized acceleration, not a commanded
    # thrust direction. Its indirect feedback through predicted P/V is retained.
    if residual is not None:
        acceleration=acceleration+residual(state.position,state.velocity,state.compensation,command)
    return acceleration,alpha


def midpoint_step(state,command,dt_s,parameters,residual=None):
    """Second-order Lie midpoint; p/v at O, R/omega in tracked coordinates."""
    h=float(dt_s);a,alpha=derivatives(state,command,parameters,residual)
    ...
    am,alpham=derivatives(midpoint,command,parameters,residual)
...
def predict_pose(initial,schedule,output_time_s,parameters,*,offset_tracking_m,maximum_step_s=.005,residual=None):
    ...
    def record(state,time):
        a,alpha=derivatives(state,schedule.sample(time-parameters.delay_s),parameters,residual)
    ...
    for start,end in zip(times[:-1],times[1:]):
        ...
        for a,b in zip(local[:-1],local[1:]):
            ...
            for _ in range(count):state=midpoint_step(state,command,h,parameters,residual)
        record(state,end)
