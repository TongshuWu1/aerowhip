...
def prepare_training(root, algorithm, *, seed, episodes, batch, device, overrides=None, resume=None, run_name=None,
    ...
    model_amendment=None
    if model_path is not None:
        if algorithm!='ppo':raise ValueError('Adapted model selection is supported for PPO only.')
        model_path=Path(model_path)
        if not model_path.is_absolute():model_path=root/model_path
        model_path=model_path.resolve();replacement=read_json(model_path)
        if keep_stopping_history:raise ValueError('A new physical model requires new convergence history.')
        if keep_optimizer_state is True:raise ValueError('A new physical model requires fresh optimizer state.')
        # Preserve the task's physical meaning; geometry changes need an explicit new setup.
        if replacement.get('recorded_data')!=configs['model'].get('recorded_data'):
            raise ValueError('Tracking/attachment geometry changed: define the new task explicitly first.')
        model_amendment=dict(source=str(model_path),source_sha256=sha256_file(model_path),
            previous_model_sha256=canonical_json_hash(configs['model']),stopping_history_reset=True,
            optimizer_state_reset=bool(resume))
        for field in ('motion_residual','fullstate_execution'):
            asset=Path(replacement[field]['checkpoint'])
            if not asset.is_absolute():replacement[field]['checkpoint']=str(model_path.parent/asset)
        configs['model']=replacement
        if resume:keep_optimizer_state=False
    ...
    atomic_json(directory / 'run.json', dict(schema='point_force_training_run_v1', algorithm=algorithm.upper(),
        ...
        model_amendment=model_amendment,
