...
class ResearchPoseModel:
    def __init__(self,path,digest,device='cuda'):
        ...
        spec=payload.get('residual',{});self.residual=None
        if spec.get('enabled',bool(spec.get('checkpoint'))):
            self.residual=load_residual(path.parent/spec['checkpoint'],spec['sha256'],device)
    ...
    def predict_differentiable(self,initial,packets,packet_times,output_times,offset,*,hover_command=None):
        """Same forward equations; preserve derivatives of state, packets and parameters.

        Timestamps, delay and the integration subdivision are scheduling data.
        Their derivatives are not approximated by silently smoothing the real
        zero-order hold. Fit delay using an outer search with exact events.
        """
        return self._predict(initial,packets,packet_times,output_times,offset,
    ...
    def _predict(self,initial,packets,packet_times,output_times,offset,*,graph,hover_command,differentiable):
        ...
        for value in (packet_times,output_times,p.delay_s,p.gravity_m_s2):
            if isinstance(value,torch.Tensor) and value.requires_grad:
                raise ValueError('Timing and gravity are fixed; fit command delay by outer search, not autograd')
        p.validate()
        packet_times=np.asarray(packet_times,dtype=float);times=np.asarray(output_times,dtype=float)
        if times.ndim!=1 or len(times)<2 or not np.isfinite(times).all() or abs(times[0]-initial.time_s)>1e-9 or np.any(np.diff(times)<=0):raise ValueError('Invalid pose output times')
        if packets.shape!=(len(initial.position),len(packet_times),11):raise ValueError('Packet dimensions differ')
        if packet_times.ndim!=1 or len(packet_times)<1 or not np.isfinite(packet_times).all() or np.any(np.diff(packet_times)<=0) or not torch.isfinite(packets).all():raise ValueError('Invalid FullState packets')
        if packets.device!=initial.position.device or packets.dtype!=initial.position.dtype:raise ValueError('Packets must match the state dtype/device')
        if torch.any(packets[:,:,10]!=0):raise ValueError('Nonzero body-rate feedforward is not supported')
        maximum=min(.005,float(torch.as_tensor(p.attitude_time_constant_s).detach())/4,
            .25/float((p.tensors(initial.position)[1]+p.tensors(initial.position)[0].sqrt()).max().detach()))
        if hover_command is None:
            hover_command=packets[:,0].clone();hover_command[:,3:9]=0.
        if hover_command.shape!=(len(packets),11) or not torch.isfinite(hover_command).all() or torch.any(hover_command[:,10]!=0):
            raise ValueError('Invalid pre-command hold')
        steps=None if differentiable else PoseStepper(initial,p,self.residual,graph=graph)
        ...
        for left,right in zip(times[:-1],times[1:]):
            ...
            record()
        if differentiable and not bool(valid.all()):
            raise ValueError('Invalid attitude/acceleration domain in differentiable prediction; no gradient is supplied')
