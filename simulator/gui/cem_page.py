...
class CEMPage(QWidget):
    def __init__(self,root,optimizer='cem'):
        ...
        browse=QPushButton('Browse seed…');browse.clicked.connect(self.browse_seed);row.addWidget(browse);body.addLayout(row);self.seed_controls=row
        split=QSplitter(Qt.Orientation.Horizontal);body.addWidget(split,1)
        from PySide6.QtWidgets import QScrollArea
        controls=QWidget();form=QFormLayout(controls);self.search_form=form
        if optimizer=='mppi':
        ...
        inspect=QPushButton('Open rehearsal & export');self.inspect_button=inspect;inspect.clicked.connect(self.inspect);resultrow.addWidget(inspect);rl.addLayout(resultrow)
    ...
    def refresh(self):
        ...
        for path in paths:
            meta=read_json(path,{})
            if meta.get('schema') not in ('research_fullstate_30hz_v1','cem_fullstate_30hz_v1','mppi_fullstate_30hz_v1','mppi_force_fullstate_30hz_v1'):continue
            self.seed.addItem(path.parent.name,str(path.parent.resolve()))
            if meta.get('schema') in (f'{self.optimizer}_fullstate_30hz_v1', 'mppi_force_fullstate_30hz_v1' if self.optimizer=='mppi' else '') and self.result_allowed(path.parent,meta):
    ...
    def start_run(self):
        ...
        try:
            ...
            command=self.prepare_planner_job(self.root,self.seed.currentData(),directory,settings,
    ...
    def draw_history(self,directory):
        ...
        axes[1].plot(x,[r.get('model_feasible_fraction',r['feasible_fraction'])*100 for r in history],label='Feasible candidates',color='#ea580c')
    ...
    def open_rehearsal(self):
        ...
        if not path:return
        if read_json(Path(path)/'rehearsal.json',{}).get('schema') not in (f'{self.optimizer}_fullstate_30hz_v1','mppi_force_fullstate_30hz_v1' if self.optimizer=='mppi' else ''):
