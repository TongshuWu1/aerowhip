...
class PointCableViewer3D(QWidget):
    ...
    def update_state(
        ...
        *, trail_phases=None, tracked_origin_m=None, tracked_rotation=None, render=True,
    ) -> None:
        """Set complete frame geometry before presenting it."""
        if (tracked_origin_m is None) != (tracked_rotation is None):
            raise ValueError('Tracked position and rotation must be supplied together.')
        ...
        if tracked_origin_m is not None:
            self.set_tracked_pose(tracked_origin_m, tracked_rotation, render=False)
        else:
            # VTK composes position with user_matrix. Discard any tracked pose
            # when returning to the legacy point-controlled representation.
            self._drone_actor.SetUserMatrix(None)
    ...
    def set_camera_preset(self, name: str) -> None:
        ...
        if getattr(self, '_scene_bounds', None) is not None:
            self.plotter.reset_camera(bounds=self._scene_bounds, render=False)
        self.plotter.reset_camera_clipping_range()
        self.plotter.render()

    def set_scene_bounds(self, bounds):
        """Fixed full-replay extent; camera presets fit it without per-frame refitting."""
        self._scene_bounds = tuple(float(value) for value in bounds)
    ...
    def set_tracked_pose(self,origin,rotation,*,render=True):
...
class MatplotlibPointCableViewer(QWidget):
    ...
    def update_state(
        ...
        *, trail_phases=None, tracked_origin_m=None, tracked_rotation=None, render=True,
        ...
        self._tracked_origin = np.asarray(tracked_origin_m) if tracked_origin_m is not None else None
        self._tracked_rotation = np.asarray(tracked_rotation) if tracked_rotation is not None else np.eye(3)
        if render:self.render()
    ...
    def _draw(self) -> None:
        ...
        origin = getattr(self, '_tracked_origin', None)
        origin = cable[0] if origin is None else origin
        rotation = getattr(self, '_tracked_rotation', np.eye(3))
        axis.scatter(*origin, color=controller_color, marker="D", s=85, label="drone" if self._live_flight else "point")
        if self._live_flight:
            for x, y in ((-.1, -.1), (-.1, .1), (.1, -.1), (.1, .1)):
                rotor = origin + rotation @ np.array([x, y, 0.])
                axis.plot(*np.vstack((origin, rotor)).T, color=controller_color, linewidth=3)
        ...
        if getattr(self, '_scene_bounds', None) is not None:
            ...
            axis.set_xlim(*b[:2]);axis.set_ylim(*b[2:4]);axis.set_zlim(*b[4:])
