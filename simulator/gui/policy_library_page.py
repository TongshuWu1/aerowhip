...
class PolicyLibraryPage(QWidget):
    ...
    def __init__(self, root):
        ...
        self.table.setHorizontalHeaderLabels(['Run name', 'Checkpoint', 'Model / rate', 'Sim. validation hit', 'Run status', 'Library'])
        ...
        self.note = QLabel('Validation is simulation-only; real-flight results are in Adaptation Check. Delete removes a checkpoint from the library. Saved files remain available for restoration.')
    ...
    def refresh(self):
        ...
        for row in rows:
            path_for_metrics=Path(row['run'])/('best_validation.json' if row['checkpoint']=='best_validation.pt' else 'validation_latest.json')
            row['metrics_revision']=path_for_metrics.stat().st_mtime_ns if path_for_metrics.exists() else 0
        if rows != self.rows:
            ...
            for index, row in enumerate(rows):
                ...
                model_label='M1' if adaptation.get('round')=='current_adp0' else ('Adapted '+str(adaptation.get('round','model')) if adaptation.get('training') else 'M0')
                ...
                for column, text in enumerate((row['name'], row['checkpoint'], model_label+' · 30 Hz · both NNs' if native else 'Legacy · saved rate',hit,row['status'],
