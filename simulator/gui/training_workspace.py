...
class AlgorithmTrainingPage(QWidget):
    ...
    def refresh_results(self):
        ...
        label=lambda m: ('adapted '+str(m['adaptation'].get('round','model'))) if m.get('adaptation',{}).get('training') else ('Unfitted baseline' if m.get('provenance',{}).get('status')=='unfitted_simulation_defaults' else 'Base model')
        selected=label(saved)+' / 30 Hz / model-specific residuals' if native else 'legacy model / original timing'
