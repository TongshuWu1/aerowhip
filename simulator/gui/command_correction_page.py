...
LABELS = dict(previous='Previous command', corrected='Corrected command', reference='Fixed reference')
...
class CommandCorrectionPage(QWidget):
    def __init__(self, root):
        ...
        self.original = QCheckBox('Show original reference command');row.addWidget(self.original)
    ...
    def poll(self, force=False):
        ...
        previous_label=v['previous'].name if v['previous'] else 'unavailable'
        if v['previous'] and v['previous'].is_relative_to(self.root/'archive'):
            previous_label='Archived intermediate command'
        self.status.setText(f"{state.upper()} · {status.get('error') or status.get('stage', 'Saved correction')}\n"
                            + f"Previous: {previous_label} → {Path(path).name}")
        ...
        self.cards[1][1].setText('Across the whip, relative to the fixed reference')
        rows = v['strike_rows']
        self.cards[2][0].setText(f'{fmt(rows[1][1])} → {fmt(rows[2][1])} cm')
        self.cards[2][1].setText(f'Relative to the fixed reference at {fmt(v["strike"], " s")}')
    ...
    def draw_commands(self):
        ...
        for name, curve in v['commands'].items():
            ...
            if name == 'reference':label = 'Original reference command'
    ...
    def draw_predictions(self):
        ...
        for col, part in enumerate(('tip', 'vehicle')):
            ...
            error_ax.set_title(title+' error relative to fixed reference', loc='left', fontsize=10, fontweight='bold')
            ...
            if reference is not None:
                path_ax.plot(reference['values'][:, 0], reference['values'][:, 2], color=COLORS['reference'], ls=':', lw=2, label='Fixed reference')
        ...
        self.explanation.setText('Previous and corrected commands are predicted using this correction run’s fitted model. '
            'The saved reference is the fixed desired motion. Dots mark the planned strike time. Simulation only; no measured hit results.'
