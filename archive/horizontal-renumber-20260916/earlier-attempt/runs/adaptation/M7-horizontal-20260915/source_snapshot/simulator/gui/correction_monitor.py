...
def snapshot(root, path):
    ...
    if provisional:
        ...
        if 'jerk_correction_coefficients_m_s3' in saved and 'command_packets' in saved:
            times=ref.get('command_time_s')
            if times is not None:
                commands['corrected']=series(times,saved['command_packets'][:len(times)],9)
        elif 'position_control_points_m' in saved:
            try:
    ...
    strike_rows = []
    for name, curve in [(meta.get('reference_label','M0')+' reference', reference['tip']),
