...
## Main checkout consolidation

All five development jobs and the validated rehearsal were copied into main.
Runtime metadata now uses repository-relative paths. Original metadata and its
hashes remain beside each copied run. CSVs, NPZ arrays, checkpoints and source
snapshots were verified byte-for-byte unchanged. No physical flight selection
was changed and no optimizer was restarted during consolidation.
