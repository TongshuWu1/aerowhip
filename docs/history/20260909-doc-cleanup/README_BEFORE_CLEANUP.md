> Historical document archived on 9 September 2026. For current work, read [HANDOFF.md](../../../HANDOFF.md). Old running-job and launch instructions below are historical.

# Documentation

Current entry points:

- [Detailed paper-writing handoff: method, contributions, evidence and experiment plan](../../PAPER_WRITING_HANDOFF.md)
- [Installation and supported environment](../../INSTALL.md)
- [Architecture](../../ARCHITECTURE.md)
- [Reproducibility and data availability](../../REPRODUCIBILITY.md)
- [Publication preparation and source release](../../PUBLICATION.md)

- [Project context and active experiments](PROJECT_CONTEXT.md)
- [Flight adaptation quickstart and ROS recorder interface](../../FLIGHT_ADAPTATION_QUICKSTART.md)
- [Supplied Mellinger controller script review and force mapping](../../CONTROLLER_INTERFACE_REVIEW.md)
- [Between-trial adaptation research proposal](../../RESEARCH_PROPOSAL_ADAPTIVE_AERIAL_WHIP.md)
- [Identification research design](../../IDENTIFICATION_RESEARCH_DESIGN.md)
- [Open-loop initial-state contract](INITIAL_STATE_OPEN_LOOP.md)
- [Paper experiment protocol](PAPER_EXPERIMENT_PROTOCOL.md)
- [Adaptation related work](../../ADAPTATION_RELATED_WORK_20260905.md)
- [Detailed literature review](../../RELATED_PAPERS_DETAILED_REVIEW.md)

Dated audits, development notes and measured/synthetic result reports are in [history/](..). They describe the settings and evidence at their original dates and are not current launch instructions. Active configuration and project context take precedence over older timing, reward, UI or fitting descriptions.
