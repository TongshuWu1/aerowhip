...
gains. The [first fit](NOMINAL_DRONE_FIT_RESULTS_20260908.md) found useful position
accuracy but attitude-domain/post-hold limitations. The subsequent
[independent attitude mapping](ATTITUDE_MAPPING_CORRECTION_20260908.md) resolves
...
remain unchanged. This engine itself does not launch fitting, training or flight.
...
The latter is used for the legacy-data probes and nominal-only fits. The same
...
For the current explicit `independent_scale_v3` nominal attitude command:
...
The two positive scales are fitted independently from maneuver orientations
while translation parameters are frozen. They describe an effective direction
mapping, not measured thrust gains. `kinematic_v1` retains u=a_O for old callers
and named comparisons. `normalized_response_v2` retains the unsuccessful
u=a_O/Ga ablation. The new fitter explicitly serializes v3, so an old candidate
is not silently reinterpreted as the new model.
...
The v3 nominal attitude target `diag(s_xy,s_xy,s_z)*a_O + g e_z` is an effective construction. With a
