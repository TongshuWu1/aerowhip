...
5. Generate a new M0 plan with the targeted-strike objective, then use
   **Rehearse + export CSV**. Inspect cable motion and the complete recovery; fold detection is diagnostic.
...
New studies freeze the objective, fold diagnostic settings, recovery, command templates and fixed
