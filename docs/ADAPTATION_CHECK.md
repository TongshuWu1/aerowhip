...
**Current evaluation convention:** all active flight scores and replay use batch hover-normalized Z. Valid calibration is required; the raw toggle is hidden. Drone/tip RMS and target distances use corrected measured geometry against the exact original saved forecast/reference target. Older numeric summaries below are historical and superseded by `docs/HOVER_HEIGHT_CALIBRATION.md` and `runs/audits/normalized-flight-evaluation/results.json`. No raw results are substituted for missing calibration, and these normalized distances are not labeled as uncorrected physical-world hitting errors.
