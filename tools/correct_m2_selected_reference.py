...
def main():
    ...
    job=ROOT/'runs/reference_tracking'/('M2-selected-local-'+stamp)
    ...
    def status(stage,**values):
        stage=stage.replace('Local M1 command correction','Local M2-selected command correction')
    ...
    try:
        ...
        status('Checking latest executed M1 command under M2-selected')
        ...
        output=ROOT/'runs/rehearsals_pva'/f'{stamp}-M2-selected-local-fixed-tip-reference'
        ...
        summary=dict(schema='pva_fullstate_30hz_v1',planner='Local M2-selected fixed-reference command correction',
        ...
        export=ROOT/'exports/M2_selected_fixed_tip_reference';export.mkdir(exist_ok=False)
        ...
        (export/'README.md').write_text(
            '# M2-selected local fixed-tip-reference command\n\n'
            ...
            'Commands are corrected with M2-selected to track the original M0 predicted tip\n'
