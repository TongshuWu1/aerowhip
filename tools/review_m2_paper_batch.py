"""Reproduce the 2026-09-13 M2 intake and frozen-forecast evaluation.
...
REHEARSAL = ROOT / 'runs/rehearsals_pva/20260913-042637-826368-M2-selected-local-fixed-tip-reference'
ROLES = {f'M2_{i:03d}': 'evaluation' for i in range(1, 6)}
...
def main():
    ...
    protocol = dict(schema=adaptation.SCHEMA, model_id='M2-selected', rehearsal=str(REHEARSAL), frozen_hashes=frozen,
        parent_generation=2, parent_forecast_model_sha256=sha256_file(REHEARSAL / 'model.json'),
        ...
        minimum_observation_fraction=.8, automatic_selection=False, neural_training=False,
        model_update='None: physical performance check only; no fitting authorized',
        role_source='User requested performance evaluation only; no future training split assigned',
        endpoints=dict(planned_strike_time_s=metadata['strike_time_s'], target_m=[1.25, 0., 1.25],
        ...
        operator_review='Pending user confirmation for this new M2 batch; no physical-contact assumption',
        qualification='Prospective M2 physical evaluation; not a paired final comparison or an M3 update batch. Clock offsets are estimates.')
