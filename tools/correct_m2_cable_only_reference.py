"""Correct executed M1 commands with the selected saved M2 variant."""
...
def main():
    ...
    source=ROOT/'runs/adaptation/M2-cable-only-20260913'
    result=read_json(source/'fit/result.json')
    if result['status']!='completed':raise ValueError('Complete M2 required')
    ...
    job=ROOT/'runs/reference_tracking'/('M2-cable-only-local-'+stamp)
    ...
    cfg.update(model_path='model.json',method='local_reference_correction',reward={},
        ...
        correction=dict(original_spline_duration_s=cfg['task']['duration_s'],fixed_handover_s=meta['interval_s'][1],
            ...
            initialization='Latest executed M1 B-spline controls; original M0 physical reference unchanged',
    ...
    sources={str(f):sha256_file(f) for f in [Path(__file__),ROOT/'planning/reference_correction.py',
        ...
        job/'settings.json',job/'reference.npz',ROOT/'runs/reference_tracking/M1-local-20260913-032256-816506/plan.npz',ROOT/'exports/M1_local_fixed_tip_reference/fullstate_30hz.csv']}
    ...
    def status(stage,**values):
        stage=stage.replace('Local M1 command correction','Local M2-cable-only command correction')
    ...
    try:
        ...
        with np.load(ROOT/'runs/reference_tracking/M1-local-20260913-032256-816506/plan.npz') as seed:
            baseline=tensor(seed['position_control_points_m'])
        cutoff=len(ref['command_time_s'])
        base_packets,base_jerk=spline.decode(baseline,origin);base_packets=base_packets[:cutoff]
        if not np.allclose(base_packets.cpu(),np.genfromtxt(ROOT/'exports/M1_local_fixed_tip_reference/fullstate_30hz.csv',delimiter=',',skip_header=1)[:cutoff,1:],atol=1e-10,rtol=0):
        ...
        status('Checking latest executed M1 command under M2-cable-only')
        ...
        output=ROOT/'runs/rehearsals_pva'/f'{stamp}-M2-cable-only-local-fixed-tip-reference'
        ...
        summary=dict(schema='pva_fullstate_30hz_v1',planner='Local M2-cable-only fixed-reference command correction',
            ...
            evidence='Simulation-only command correction toward the original M0 physical motion; M2 has not flown.',
        ...
        export=ROOT/'exports/M2_cable_only_fixed_tip_reference';export.mkdir(exist_ok=False)
        ...
        (export/'README.md').write_text(
            '# M2-cable-only local fixed-tip-reference command\n\n'
            ...
            'Commands are corrected with M2-cable-only to track the original M0 predicted tip\n'
